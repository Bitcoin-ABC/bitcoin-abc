...
export const sendXec = async (
    ...
    for (const targetOutput of targetOutputs) {
        if ('script' in targetOutput) {
            // If script is specified, convert to expected Script
            outputs.push({
                value: targetOutput.value,
                script: new Script(targetOutput.script),
            });
            continue;
        }
        // If we have no address and no script, assign change address
        // This prepares targetOutputs generated by slpv1 targetOutput buidling functions for ecash-lib
        // TODO will update the targetOutput functions to correctly prepare these outputs when
        // slp-mdm is deprecated
        if (
...
export const getMaxSendAmountSatoshis = (
    ...
    // Prepare OP_RETURN output for ecash-lib
    // TODO when we deprecate utxo-lib OP_RETURN methods, this will be done already
    if (
        typeof scriptOutputs[0] !== 'undefined' &&
        'script' in scriptOutputs[0]
    ) {
        // If script is specified, convert to expected Script
        scriptOutputs[0].script = new Script(scriptOutputs[0].script);
    }
