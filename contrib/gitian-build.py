    ...
    if args.linux:
        ...
        subprocess.check_call(
            [
                ...
                "--url",
                "bitcoin=" + args.url,
                "../bitcoin-abc/contrib/gitian-descriptors/gitian-linux.yml",
            ]
        )
        subprocess.check_call(
            [
    ...
    if args.windows:
        ...
        subprocess.check_call(
            [
                ...
                "--url",
                "bitcoin=" + args.url,
                "../bitcoin-abc/contrib/gitian-descriptors/gitian-win.yml",
            ]
        )
        subprocess.check_call(
            [
        ...
        subprocess.check_call(
            "mv build/out/bitcoin-*.zip build/out/bitcoin-*.exe " + output_dir_win,
            shell=True,
        )
        subprocess.check_call(
            "mv build/out/src/bitcoin-*.tar.gz " + output_dir_src, shell=True
    ...
    if args.macos:
        ...
        subprocess.check_call(
            [
                ...
                "--url",
                "bitcoin=" + args.url,
                "../bitcoin-abc/contrib/gitian-descriptors/gitian-osx.yml",
            ]
        )
        subprocess.check_call(
            [
        ...
        subprocess.check_call(
            "mv build/out/bitcoin-*.tar.gz build/out/bitcoin-*.dmg " + output_dir_osx,
            shell=True,
        )
        subprocess.check_call(
            "mv build/out/src/bitcoin-*.tar.gz " + output_dir_src, shell=True
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call(
            "cp inputs/bitcoin-"
            + args.version
            + "-win-unsigned.tar.gz inputs/bitcoin-win-unsigned.tar.gz",
            shell=True,
        )
        subprocess.check_call(
            [
        ...
        subprocess.check_call(
            "mv build/out/bitcoin-*win64-setup.exe ../bitcoin-binaries/" + args.version,
            shell=True,
        )
    ...
    if args.macos:
        ...
        subprocess.check_call(
            "cp inputs/bitcoin-"
            + args.version
            + "-osx-unsigned.tar.gz inputs/bitcoin-osx-unsigned.tar.gz",
            shell=True,
        )
        subprocess.check_call(
            [
        ...
        subprocess.check_call(
            "mv build/out/bitcoin-osx-signed.dmg ../bitcoin-binaries/"
            + args.version
            + "/bitcoin-"
            + args.version
            + "-osx.dmg",
    ...
    if args.commit_files:
        ...
        subprocess.check_call(
            [
                ...
                "-a",
                "-m",
                "Add " + args.version + " signed binary sigs for " + args.signer,
            ]
        )
        os.chdir(workdir)
...
def main():
    ...
    parser.add_argument(
        ...
        dest="pull",
        help=(
            "Indicate that the version argument is the number of a github repository"
            " pull request"
        ),
    )
    parser.add_argument(
        ...
        "--url",
        dest="url",
        default="https://github.com/Bitcoin-ABC/bitcoin-abc.git",
    ...
    parser.add_argument(
        ...
        default="lwm",
        help=(
            "Specify which Operating Systems the build is for. Default is %(default)s."
            " l for Linux, w for Windows, m for MacOS"
        ),
    )
    parser.add_argument(
        "-j",
    ...
    parser.add_argument(
        ...
        dest="setup",
        help=(
            "Set up the Gitian building environment. Uses LXC. If you want to use KVM,"
            " use the --kvm option. Only works on Debian-based systems (Ubuntu, Debian)"
        ),
    )
    parser.add_argument(
        "-D",
    ...
    parser.add_argument(
        "version",
        help=(
            "Version number, commit, or branch to build. If building a commit or"
            " branch, the -c option must be specified"
        ),
    )
    ...
    if args.macos and not os.path.isfile(
        "gitian-builder/inputs/Xcode-12.1-12A7403-extracted-SDK-with-libcxx-headers.tar.gz"
    ):
    ...
    if args.pull:
        ...
        args.commit = subprocess.check_output(
            ["git", "show", "-s", "--format=%H", "FETCH_HEAD"],
