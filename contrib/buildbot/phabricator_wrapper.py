...
class PhabWrapper(Phabricator):
    ...
    def get_revisionPHID(self, branch):
        ...
        assert len(data_list) == 1, "differential.diff.search({}): Expected 1 diff, got: {}".format(
    ...
    def get_revision_info(self, revisionPHID):
        ...
        data_list = self.differential.revision.search(
    ...
    def getRevisionAuthor(self, revisionId):
        ...
        assert len(rev_list) == 1, "differential.revision.search({}): Expected 1 revision, got: {}".format(
        ...
        assert len(author_list) == 1, "user.search({}): Expected 1 user, got: {}".format(
            userSearchArgs, author_list)
    ...
    def getRevisionPHIDsFromCommits(self, commitHashes):
        ...
        assert len(commits) == expectedNumCommits, "diffusion.commit.search({}): Expected {} commits, got: {}".format(
    ...
    def decorateCommitMap(self, commitMapIn):
        ...
        assert len(revs) == len(revisionPHIDs), "differential.revision.search({}): Expected {} revisions, got: {}".format(
            revisionSearchArgs, len(revisionPHIDs), revs)

        # Decorate revision authors
        authorPHIDs = [rev['fields']['authorPHID'] for rev in revs]
        authors = self.user.search(constraints={
            'phids': authorPHIDs,
        }).data
            ...
            if revisionPHID:
                for rev in revs:
                    if revisionPHID == rev['phid']:
                        ...
                        decoratedCommit['link'] = "https://reviews.bitcoinabc.org/D{}".format(
                            rev['id'])
                        break
                ...
                for author in authors:
                    if author['phid'] == rev['fields']['authorPHID']:
                        ...
                        decoratedCommit['authorSlackUsername'] = self.getAuthorSlackUsername(
                            author)
                        break
    ...
    def commentOnRevision(self, revisionID, msg, buildName=""):
        ...
        if self.deployment == Deployment.PROD and BUILDNAME_IGNORE_KEYWORD not in buildName:
    ...
    def createBrokenBuildTask(
        ...
        newTask = self.maniphest.edit(transactions=[
    ...
    def get_project_members(self, project_PHID):
        ...
        return [m["phid"]
                for m in project_data[0]["attachments"]["members"]["members"]]
    ...
    def get_revision_changed_files(self, revision_id):
        return list(self.differential.getcommitpaths(
            revision_id=revision_id).values())
    ...
    def get_file_content_from_master(self, path):
        ...
        if path in self.file_cache and self.file_cache[path]['commit'] == latest_commit_hash:
            return self.file_cache[path]['content']

        def file_not_found(data):
            raise AssertionError("File {} not found in master commit {} for repository {}:\n{}".format(
                path,
                latest_commit_hash,
                BITCOIN_ABC_REPO,
                data,
        ...
        if file_data["tooSlow"] or file_data["tooHuge"]:
            raise AssertionError("File {} from commit {} for repository {} is oversized or took too long to download: {}".format(
                path,
                latest_commit_hash,
                BITCOIN_ABC_REPO,
                file_data,
        ...
        file_content = self.file.download(
            phid=file_data["filePHID"]
        ).response

        if not file_content:
            self.logger.info("File {} appear to be empty in commit {} for repository {}".format(
                file_data["filePHID"],
                latest_commit_hash,
                BITCOIN_ABC_REPO,
            ))
