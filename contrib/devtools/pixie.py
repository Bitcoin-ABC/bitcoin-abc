...
class ELFRecord(types.SimpleNamespace):
    ...
    def __init__(self, data: bytes, offset: int, eh: 'ELFHeader',
                 total_size: Optional[int]) -> None:
        hdr_struct = self.STRUCT[eh.ei_class][0][eh.ei_data]
        if total_size is not None and hdr_struct.size > total_size:
            raise ValueError(
                f'{self.__class__.__name__} header size too small ({total_size} < {hdr_struct.size})')
        for field, value in zip(self.STRUCT[eh.ei_class][1], hdr_struct.unpack(
                data[offset:offset + hdr_struct.size])):
...
class Symbol(ELFRecord):
    ...
    def __init__(self, data: bytes, offset: int, eh: ELFHeader,
                 symtab: Section, strings: bytes, version: Optional[bytes]) -> None:
...
class Vernaux(ELFRecord):
    DEF = (BiStruct('IHHII'), ['vna_hash', 'vna_flags',
           'vna_other', 'vna_name', 'vna_next'])
    STRUCT = {ELFCLASS32: DEF, ELFCLASS64: DEF}
...
class DynTag(ELFRecord):
    ...
    def __init__(self, data: bytes, offset: int,
                 eh: ELFHeader, section: Section) -> None:
...
def _parse_symbol_table(section: Section, strings: bytes, eh: ELFHeader,
                        versym: bytes, verneed: Dict[int, bytes]) -> List[Symbol]:
...
def _parse_dyn_tags(section: Section, strings: bytes,
                    eh: ELFHeader) -> List[Tuple[int, Union[bytes, int]]]:
    ...
    for ofs in range(0, len(data), section.sh_entsize):
        ...
        val = _lookup_string(
            strings, tag.d_val) if tag.d_tag in STRING_TAGS else tag.d_val
