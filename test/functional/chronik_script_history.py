...
class ChronikScriptHistoryTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            chronik.script('', '').history().err(400).msg,
        ...
        assert_equal(
            ...
            .history(page=0, page_size=201).err(400).msg,
            '400: Requested page size 201 is too big, maximum is 200')
        assert_equal(
            chronik.script('p2pk', GENESIS_CB_PK)
            .history(page=0, page_size=0).err(400).msg,
            '400: Requested page size 0 is too small, minimum is 1')
        assert_equal(
            chronik.script('p2pk', GENESIS_CB_PK)
            .history(page=0, page_size=2**32).err(400).msg,
            '400: Invalid param page_size: 4294967296, ' +
            'number too large to fit in target type')
        assert_equal(
            chronik.script('p2pk', GENESIS_CB_PK)
            .history(page=2**32, page_size=1).err(400).msg,
        ...
        def check_tx_history(mempooltxs, blocktxs, *, page_size=25):
            ...
            for page_num, page_txs in enumerate(pages):
                script_history = (
                    ...
                    .history(page=page_num, page_size=page_size).ok()
                )
        ...
        mempooltxs = sorted([{'time_first_seen': mocktime + offset, 'txid': txid}
                             for (offset, txid)
                             in zip(mocktime_offsets, mempool_txids)],
                            key=tx_sort_key,
                            reverse=True)
        ...
        newblocktxs = [entry for entry in mempooltxs if entry['txid']
                       not in mempool_txids[:5]]
        ...
        coinbase_tx.vout[0].scriptPubKey = P2SH_OP_TRUE
        coinbase_tx.rehash()
        block = create_block(int(blockhashes[-1], 16),
                             coinbase_tx,
                             mocktime + 1100)
        block.nVersion = 5
        block.vtx += mine_txs
        ...
        txs_individually = [
            chronik.script(
                script_type, payload_hex).history(page=i, page_size=1).ok().txs[0]
            for i in range(20)
        ]
        txs_bulk = list(chronik.script(
            script_type, payload_hex).history(page=0, page_size=20).ok().txs)
        # Contain the same txs, but not necessarily in the same order
        assert_equal(sorted(txs_individually, key=lambda tx: tx.txid),
