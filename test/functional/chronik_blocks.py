...
class ChronikBlockRangeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(chronik.blocks(-1, 0).err(400).msg,
                     '400: Invalid block start height: -1')
        assert_equal(chronik.blocks(-2**31, 0).err(400).msg,
                     f'400: Invalid block start height: {-2**31}')
        assert_equal(chronik.blocks(2, 1).err(400).msg,
                     '400: Invalid block end height: 1')
        assert_equal(
