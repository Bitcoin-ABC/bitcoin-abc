...
class ChronikScriptUtxosTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            chronik.script('', '').utxos().err(400).msg,
        ...
        assert_equal(chronik.script('p2pk', GENESIS_CB_PK).utxos().ok(),
                     pb.ScriptUtxos(script=bytes.fromhex(f'41{GENESIS_CB_PK}ac'),
                                    utxos=[pb.ScriptUtxo(
        ...
        assert_equal(chronik.script(script_type, payload_hex).utxos().ok(),
                     pb.ScriptUtxos(script=bytes(P2SH_OP_TRUE),
                                    utxos=[pb.ScriptUtxo(
                                        outpoint=pb.OutPoint(
        ...
        tx2 = CTransaction()
        tx2.vin = [CTxIn(outpoint=COutPoint(int(txid, 16), 3),
                         scriptSig=SCRIPTSIG_OP_TRUE)]
        ...
        expected_utxos.append(pb.ScriptUtxo(
            outpoint=pb.OutPoint(
        ...
        tx3 = CTransaction()
        tx3.vin = [CTxIn(outpoint=COutPoint(int(txid, 16), 2),
                         scriptSig=SCRIPTSIG_OP_TRUE),
                   CTxIn(outpoint=COutPoint(int(txid2, 16), 0),
                         scriptSig=SCRIPTSIG_OP_TRUE)]
        pad_tx(tx3)
        ...
        assert_equal(list(chronik.script(script_type, payload_hex).utxos().ok().utxos),
