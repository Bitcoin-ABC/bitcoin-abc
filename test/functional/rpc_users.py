...
class HTTPBasicsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            401, call_with_auth(self.nodes[0], 'rtwrong', password).status)

        # Wrong password for rt
        self.log.info('Wrong...')
        assert_equal(
            401, call_with_auth(self.nodes[0], 'rt',
                                password + 'wrong').status)

        # Correct for rt2
        self.log.info('Correct...')
        assert_equal(
            200, call_with_auth(self.nodes[0], 'rt2', password2).status)

        # Wrong password for rt2
        self.log.info('Wrong...')
        assert_equal(
            401, call_with_auth(self.nodes[0], 'rt2',
                                password2 + 'wrong').status)

        # Correct for randomly generated user
        self.log.info('Correct...')
        assert_equal(
            200, call_with_auth(self.nodes[0], self.user,
        ...
        self.log.info('Correct...')
        assert_equal(
            200, call_with_auth(self.nodes[1], "rpcuser💻",
                                "rpcpassword🔑").status)
        ...
        self.log.info('Wrong...')
        assert_equal(
            401, call_with_auth(self.nodes[1], 'rpcuser',
