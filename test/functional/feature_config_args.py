...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_invalid_command_line_options(self):
        self.nodes[0].assert_start_raises_init_error(
            expected_msg='Error: No proxy server specified. Use -proxy=<ip> or '
                         '-proxy=<ip:port>.',
            extra_args=['-proxy'],
        )
    ...
    def test_seed_peers(self):
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=[
            ...
            self.start_node(
                0, extra_args=[
                    '-dnsseed=1 -mocktime={}'.format(start)])
        with self.nodes[0].assert_debug_log(expected_msgs=[
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=[
            ...
            self.start_node(
                ...
                extra_args=[
                    '-dnsseed=0',
                    '-addnode=fakenodeaddr -mocktime={}'.format(start)])
        with self.nodes[0].assert_debug_log(expected_msgs=[
                "Adding fixed seeds as 60 seconds have passed and addrman is empty"]):
            self.nodes[0].setmocktime(start + 65)
        self.stop_node(0)
    ...
    def run_test(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            ['-conf=' + conf_file], 'Error: Error reading configuration file: specified data directory "' + new_data_dir + '" does not exist.')
