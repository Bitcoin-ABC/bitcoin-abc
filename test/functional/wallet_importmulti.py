...
class ImportMultiTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        key = get_key(self.nodes[0])
        self.test_importmulti(
            {"scriptPubKey": key.p2pkh_script, "timestamp": "now", "internal": True},
            success=True,
        )
        test_address(
            self.nodes[1],
        ...
        self.log.info(
            "Should not import a nonstandard scriptPubKey without internal and with"
            " public key"
        )
        key = get_key(self.nodes[0])
        self.test_importmulti(
        ...
        self.test_importmulti(
            ...
            success=True,
            warnings=[
                "All private keys are provided, outputs will be considered spendable."
        ...
        self.test_importmulti(
            {"scriptPubKey": {"address": multisig.p2sh_addr}, "timestamp": "now"},
            success=True,
        )
        test_address(
            self.nodes[1],
        ...
        self.log.info(
            "Should import a scriptPubKey with internal and with a wrong public key as"
            " non-solvable"
        )
        key = get_key(self.nodes[0])
        wrong_key = get_key(self.nodes[0]).pubkey
        ...
        self.log.info(
            "Should import a scriptPubKey with internal and with a wrong private key as"
            " non-solvable"
        )
        key = get_key(self.nodes[0])
        wrong_privkey = get_key(self.nodes[0]).privkey
        ...
        self.test_importmulti(
            {"scriptPubKey": {"address": watchonly_address}, "timestamp": "now"},
            success=True,
        )
        test_address(
            self.nodes[1],
        ...
        self.test_importmulti(
            {"desc": descsum_create(desc), "timestamp": "now", "range": 1}, success=True
        )
        ...
        test_address(self.nodes[1], key1.p2pkh_addr, ismine=False, iswatchonly=False)

        # Import pubkeys with key origin info
        self.log.info(
            "Addresses should have hd keypath and master key id after import with key"
            " origin"
        )
        ...
        self.log.info(
            "Imported scripts with pubkeys shoud not have their pubkeys go into the"
            " keypool"
        )
        addr1 = self.nodes[0].getnewaddress()
        addr2 = self.nodes[0].getnewaddress()
