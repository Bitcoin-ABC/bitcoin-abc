...
class ChronikTokenSlpNft1(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        mint = TokenTx(
            ...
            outputs=[
                pb.Token(),
                group_token(token_id=genesis.txid, atoms=20),
                pb.Token(),
        ...
        send = TokenTx(
            ...
            inputs=[
                group_token(token_id=genesis.txid, atoms=5000),
            ],
            outputs=[
                pb.Token(),
                group_token(token_id=genesis.txid, atoms=1),
                group_token(token_id=genesis.txid, atoms=99),
                group_token(token_id=genesis.txid, atoms=900),
                group_token(token_id=genesis.txid, atoms=4000),
            ],
        )
        txs.append(send)
        send.send(chronik)
