...
def rpccall(node, user, method):
    ...
    headers = {"Authorization": f"Basic {str_to_b64str(f'{user[0]}:{user[3]}')}"}
...
class RPCWhitelistTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for permission in self.never_allowed:
            ...
            assert_equal(
                403, rpccall(self.nodes[0], self.strange_users[0], permission).status
            )
        for permission in self.never_allowed:
            self.log.info("Strange test 2")
            assert_equal(
                403, rpccall(self.nodes[0], self.strange_users[1], permission).status
            )
        self.log.info("Strange test 3")
        assert_equal(
            200, rpccall(self.nodes[0], self.strange_users[2], "getblockcount").status
        )
        self.log.info("Strange test 4")
        assert_equal(
            403,
            rpccall(self.nodes[0], self.strange_users[3], "getbestblockhash").status,
        )
        self.log.info("Strange test 5")
        assert_equal(
            200, rpccall(self.nodes[0], self.strange_users[4], "getblockcount").status
        )
