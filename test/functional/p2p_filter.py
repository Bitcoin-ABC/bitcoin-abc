...
class FilterTest(BitcoinTestFramework):
    ...
    def generatetoscriptpubkey(self, scriptpubkey):
        ...
        return self.generatetodescriptor(
            self.nodes[0], 1, f'raw({scriptpubkey.hex()})')[0]

    def test_size_limits(self, filter_peer):
        self.log.info('Check that too large filter is rejected')
        with self.nodes[0].assert_debug_log(['Misbehaving']):
            filter_peer.send_and_ping(msg_filterload(
        ...
        with self.nodes[0].assert_debug_log(['Misbehaving']):
            filter_peer.send_and_ping(msg_filteradd(
    ...
    def test_msg_mempool(self):
        ...
        txid, _ = self.wallet.send_to(
            from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=9 * COIN)
