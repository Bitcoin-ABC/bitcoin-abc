...
class HelpRpcTest(BitcoinTestFramework):
    ...
    def test_client_conversion_table(self):
        ...
        mapping_server_conversion = [tuple(m[:3])
                                     for m in mapping_server if not m[3]]

        # Only check if all RPC methods have been compiled (i.e. wallet is
        # enabled)
        if self.is_wallet_compiled() and sorted(
                mapping_client) != sorted(mapping_server_conversion):
            raise AssertionError(
                f"RPC client conversion table ({file_conversion_table}) and "
                f"RPC server named arguments mismatch!\n"
        ...
        for argname, convert in converts_by_argname.items():
            if all(convert) != any(convert):
                ...
                assert argname == 'dummy', (
                    f'WARNING: conversion mismatch for argument named '
