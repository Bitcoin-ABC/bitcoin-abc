...
from test_framework.p2p import DISK_MAGIC_BYTES
...
class ReindexTest(BitcoinTestFramework):
    ...
    def out_of_order(self):
        ...
        with open(blk0, "r+b") as bf:
            ...
            def find_block(b, start):
                return b.find(DISK_MAGIC_BYTES["regtest"], start) + 4
    ...
    def continue_reindex_after_shutdown(self):
        ...
        with node.assert_debug_log(
            expected_msgs=[f"Opening LevelDB in {db_path}"],
            unexpected_msgs=[f"Wiping LevelDB in {db_path}"],
        ):
