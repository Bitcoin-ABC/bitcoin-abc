...
BLOCKS_VALUE_OF_ZERO = (
    "error: the first argument (number of blocks to generate, default: 1) must be an"
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            BLOCKS,
            self.nodes[0]
            .cli(f"-rpcuser={user}", "-stdinrpcpass", cli_input=password)
            .getblockcount(),
        )
        assert_raises_process_error(
            1,
            ...
            self.nodes[0]
            .cli(f"-rpcuser={user}", "-stdinrpcpass", cli_input="foo")
            .echo,
        )
        ...
        assert_raises_process_error(
            ...
            "Incorrect rpcuser or rpcpassword",
            self.nodes[0]
            .cli(f"-rpcuser={user}", "-stdin", "-stdinrpcpass", cli_input="foo")
            .echo,
        )
        ...
        assert_raises_process_error(
            ...
            self.nodes[0].cli("-rpccookiefile=does-not-exist", "-rpcpassword=").echo,
        )
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info(
                "Test -getinfo with multiple wallets and -rpcwallet returns specified"
                " wallet balance"
            )
            for i in range(len(wallets)):
                cli_get_info = (
                    self.nodes[0].cli("-getinfo", f"-rpcwallet={wallets[i]}").send_cli()
                )
                assert "balances" not in cli_get_info.keys()
            ...
            cli_get_info_keys = (
                self.nodes[0].cli("-getinfo", rpcwallet3).send_cli().keys()
            )
            assert "balance" not in cli_get_info_keys
            assert "balances" not in cli_get_info_keys
            ...
            assert_raises_process_error(
                1, JSON_PARSING_ERROR, self.nodes[0].cli("-generate", "foo").echo
            )
            assert_raises_process_error(
            ...
            assert_raises_rpc_error(
                -18, WALLET_NOT_LOADED, self.nodes[0].cli(rpcwallet3, "-generate").echo
            )
            assert_raises_rpc_error(
                -18,
                WALLET_NOT_LOADED,
            ...
            generate = (
                self.nodes[0].cli(rpcwallet2, "-generate", n4, 1000000).send_cli()
            )
            assert_equal(set(generate.keys()), {"address", "blocks"})
            ...
            assert_raises_rpc_error(
                -19, WALLET_NOT_SPECIFIED, self.nodes[0].cli("-generate", "foo").echo
            )
            assert_raises_rpc_error(
                -19, WALLET_NOT_SPECIFIED, self.nodes[0].cli("-generate", 0).echo
            )
            assert_raises_rpc_error(
                -19, WALLET_NOT_SPECIFIED, self.nodes[0].cli("-generate", 1, 2, 3).echo
            )
        else:
            self.log.info(
                "*** Wallet not compiled; cli getwalletinfo and -getinfo wallet tests"
                " skipped"
            )
