...
class P2PNobloomfilterMessages(BitcoinTestFramework):
    ...
    def test_message_causes_disconnect(self, message):
        # Add a p2p connection that sends a message and check that it
        # disconnects
    ...
    def run_test(self):
        self.log.info(
            "Test that node is disconnected if it sends mempool message")
        self.test_message_causes_disconnect(msg_mempool())

        self.log.info(
            "Test that node is disconnected if it sends filterload message")
        self.test_message_causes_disconnect(msg_filterload())

        self.log.info(
            "Test that node is disconnected if it sends filteradd message")
