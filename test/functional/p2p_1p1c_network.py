...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def raise_network_minfee(self):
        ...
        self.log.debug(
            "Check that all nodes' mempool minimum feerates are above min relay feerate"
        )
    ...
    def create_basic_1p1c(self, wallet):
        low_fee_parent = wallet.create_self_transfer(
            fee_rate=FEERATE_1SAT_VB, confirmed_only=True
        )
    ...
    def create_package_2outs(self, wallet):
        ...
        # Round up. The goal is to be between min relay feerate and mempool min
        # feerate.
        fee_2outs = ceil(low_fee_parent_2outs_tester["tx"].billable_size() / 2)
    ...
    def create_package_2p1c(self, wallet):
        mempool_min_fee = self.nodes[0].getmempoolinfo()["mempoolminfee"]
        parent1 = wallet.create_self_transfer(
            fee_rate=mempool_min_fee * 2, confirmed_only=True
        )
        parent2 = wallet.create_self_transfer(
            fee_rate=mempool_min_fee * 3, confirmed_only=True
        )
        child = wallet.create_self_transfer_multi(
            utxos_to_spend=[parent1["new_utxo"], parent2["new_utxo"]],
            fee_per_output=999 * parent1["tx"].billable_size(),
        )
        return (
            [parent1["hex"], parent2["hex"], child["hex"]],
            parent1["tx"],
            parent2["tx"],
            child["tx"],
        )
    ...
    def create_packages(self):
        ...
        package_hex_2, parent_2, child_2 = self.create_basic_1p1c(self.wallet_p2pk)
        ...
        package_hex_3, parent_31, parent_32, child_3 = self.create_package_2p1c(
            self.wallet
        )
    ...
    def run_test(self):
        ...
        self.wallet_p2pk = MiniWallet(self.nodes[2], mode=MiniWalletMode.RAW_P2PK)
        self.generate(self.wallet_p2pk, 10)
        self.generate(self.wallet, 120)

        self.log.info(
            "Fill mempools with large transactions to raise mempool minimum feerates"
        )
