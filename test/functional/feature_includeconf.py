...
class IncludeConfTest(BitcoinTestFramework):
    ...
    def run_test(self):
        self.log.info(
            "-includeconf works from config file. subversion should end with 'main;"
            " relative)/'"
        )
        ...
        self.log.info(
            "-includeconf cannot be used recursively. subversion should end with 'main;"
            " relative)/'"
        )
        with open(
            os.path.join(self.options.tmpdir, "node0", "relative.conf"),
        ...
        self.log.info(
            "multiple -includeconf args can be used from the base config file."
            " subversion should end with 'main; relative; relative2)/'"
        )
        with open(
            os.path.join(self.options.tmpdir, "node0", "relative.conf"),
            "w",
