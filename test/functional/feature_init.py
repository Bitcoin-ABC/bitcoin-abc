...
class InitStressTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for terminate_line in lines_to_terminate_after:
            ...
            while True:
                ...
                if line and terminate_line.lower() in line.lower():
                    self.log.debug(
                        f"Terminating node after {num_lines} log lines seen")
                    sigterm_node()
                    break
                ...
                if node.process.poll() is not None:
                    raise AssertionError(
                        f"node failed to start (line: '{terminate_line}')")
        ...
        for _ in range(40):
            ...
            self.log.debug(
                f"Starting node and will exit after {terminate_after} lines")
            node.start(
                # TODO: add -txindex=1 to fully test index initiatlization.
                # extra_args=['-txindex=1'],
            )
            ...
            while True:
                ...
                if num_lines >= terminate_after or (
        ...
        for file_patt, err_fragment in files_to_disturb.items():
            ...
            #
            # At the moment I can't get this to work (bitcoind loads
