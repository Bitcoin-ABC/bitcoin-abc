...
struct chainparams {
	...
	const struct xec_blkid genesis_blockhash;
	const int rpc_port;
	/**
	 * BOLT 1:
	 *
	 ...
	 *
	 * - xec mainet with port number 9735 or the corresponding hexadecimal `0x2607`;
	 * - xec testnet with port number 19735 (`0x4D17`);
	 * - xec signet with port number 39735 (`0xF87`).
	 */
	const int ln_port;
	const char *cli;
	const char *cli_args;
