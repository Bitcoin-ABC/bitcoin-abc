...
static void peer_got_announcement(struct channel *channel, const u8 *msg)
{
	...
	secp256k1_ecdsa_signature remote_ann_xec_sig;
			 ...
			 bool reestablish_only)
{
	...
	secp256k1_ecdsa_signature *remote_ann_node_sig, *remote_ann_xec_sig;
			...
			u32 depth)
{
	...
	txidstr = type_to_string(tmpctx, struct xec_txid, txid);
	channel->depth = depth;
			      ...
			      u32 block_height)
{
	...
	for (i = 0; i < tal_count(to_forget); ++i) {
		...
		log_unusual(channel->log,
			    ...
			    type_to_string(tmpctx, struct xec_txid,
...
/* Since this could vanish while we're checking with xecd, we need to save
...
static void process_check_funding_broadcast(struct xecd *xecd,
					    const struct xec_tx_output *txout,
					    void *arg)
{
	...
	peer = peer_by_id(xecd->ld, &cc->peer);
	if (!peer)
		goto cleanup;
						       ...
						       struct peer *peer)
{
	 ...
	 * been onchain. */
	xecd_getutxout(cmd->ld->topology->xecd,
