...
new_inflight(struct channel *channel,
	     const struct xec_outpoint *funding_outpoint,
	     u32 funding_feerate,
	     struct amount_sat total_funds,
	     struct amount_sat our_funds,
	     struct wally_psbt *psbt STEALS,
	     struct xec_tx *last_tx,
	     const struct xec_signature last_sig,
	     ...
	     const struct amount_sat lease_amt)
{
	...
	inflight->last_tx = xec_tx_with_psbt(inflight, last_tx_psbt_clone);
...
struct channel *new_channel(struct peer *peer, u64 dbid,
			    ...
			    u64 next_htlc_id,
			    const struct xec_outpoint *funding,
			    ...
			    const struct xec_signature *last_sig,
			    /* NULL or stolen */
			    const struct xec_signature *last_htlc_sigs,
			    ...
			    /* NULL or stolen */
			    const struct xec_outpoint *shutdown_wrong_funding,
					       ...
					       const struct xec_txid *txid)
{
	...
	list_for_each(&channel->inflights, inflight, list) {
		if (xec_txid_eq(txid, &inflight->funding->outpoint.txid))
			return inflight;
	}
