...
struct outgoing_tx {
	...
	bool (*refresh)(struct channel *, const struct xec_tx **, void *arg);
	void *refresh_arg;
};
...
struct block {
	...
	struct xec_tx **full_txs;
	struct xec_txid *txids;
};

/* Hash blocks by sha */
static inline const struct xec_blkid *keyof_block_map(const struct block *b)
{
	return &b->blkid;
}

static inline size_t hash_sha(const struct xec_blkid *key)
{
	size_t ret;
...
static inline bool block_eq(const struct block *b, const struct xec_blkid *key)
{
	return xec_blkid_eq(&b->blkid, key);
}
HTABLE_DEFINE_TYPE(struct block, keyof_block_map, hash_sha, block_eq, block_map);

/* Hash blocks by sha */
static inline const struct xec_txid *keyof_outgoing_tx_map(const struct outgoing_tx *t)
{
	return &t->txid;
}

static inline size_t outgoing_tx_hash_sha(const struct xec_txid *key)
{
	size_t ret;
	memcpy(&ret, key, sizeof(ret));
	return ret;
...
static inline bool outgoing_tx_eq(const struct outgoing_tx *b, const struct xec_txid *key)
{
	return xec_txid_eq(&b->txid, key);
}
...
struct chain_topology {
	...
	struct block *root;
	struct block *tip;
	struct xec_blkid prev_tip;
	struct block_map *block_map;

	/* Set during startup */
	bool feerate_uninitialized;

	/* This is the lowest feerate that xecd is saying will broadcast. */
	u32 feerate_floor;
	...
	/* struct sync_waiters waiting for us to catch up with xecd (and
	...
	/* xec transactions we're broadcasting */
	...
	/* The number of headers known to the xec backend at startup. Not
...
/* Get the minimum feerate that xecd will accept */
 ...
 * in the xec backend as well. If an absolute time is required, rather
...
/* Usually we set nLocktime to tip (or recent) like xecd does */
...
void broadcast_tx_(struct chain_topology *topo,
		   struct channel *channel,
		   const struct xec_tx *tx TAKES,
		   const char *cmd_id, bool allowhighfees, u32 minblock,
		   void (*finished)(struct channel *,
				    bool success,
				    const char *err),
		   bool (*refresh)(struct channel *, const struct xec_tx **, void *),
...
struct txlocator *locate_tx(const void *ctx, const struct chain_topology *topo, const struct xec_txid *txid);
...
/**
 * topology_add_sync_waiter: wait for lightningd to sync with xec network
