...
openchannel2_sign_hook_cb(struct openchannel2_psbt_payload *payload STEALS)
{
	...
	if (!xec_txid_eq(&inflight->funding->outpoint.txid, &txid)) {
		log_broken(channel->log,
...
wallet_update_channel(struct lightningd *ld,
		      struct channel *channel,
		      struct xec_tx *remote_commit STEALS,
		      struct xec_signature *remote_commit_sig,
...
static void check_utxo_block(struct xecd *xecd UNUSED,
...
static void sendfunding_done(struct xecd *xecd UNUSED,
			     ...
			     struct channel_send *cs)
{
	...
	if (success) {
		...
		xecd_getutxout(ld->topology->xecd,
				   &channel->funding,
				   check_utxo_block,
				   cs);
	}
			    ...
			    const struct wally_tx *wtx TAKES)
{
	...
	log_debug(channel->log,
		  ...
		  type_to_string(tmpctx, struct xec_txid, &txid),
		  type_to_string(tmpctx, struct channel_id, &channel->cid),
		  type_to_string(tmpctx, struct wally_tx, cs->wtx));

	xecd_sendrawtx(ld->topology->xecd,
			 ...
			 u32 depth)
{
	...
	if (to_go == 0) {
		assert(channel->scid);
		assert(xec_txid_eq(&channel->funding.txid, txid));
		      ...
		      const jsmntok_t *params)
{
	...
	if (!topology_synced(cmd->ld->topology)) {
		return command_fail(cmd, FUNDING_STILL_SYNCING_xec,
				    "Still syncing with xec network");
	}
			 ...
			 const jsmntok_t *params)
{
	...
	if (!xec_txid_eq(&txid, &channel->funding.txid))
		return command_fail(cmd, FUNDING_PSBT_INVALID,
...
static void validate_input_unspent(struct xecd *xecd,
				   const struct xec_tx_output *txout,
				   void *arg)
{
	...
	/* First time thru xecd will be NULL, otherwise is response */
	if (xecd && !txout) {
		struct xec_outpoint outpoint;
	...
	for (size_t i = pv->next_index; i < pv->psbt->num_inputs; i++) {
		...
		xecd_getutxout(pv->channel->owner->ld->topology->xecd,
				   &outpoint,
						    ...
						    const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt("close_to", param_xec_address, &our_upfront_shutdown_script),
