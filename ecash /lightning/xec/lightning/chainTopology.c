...
static void filter_block_txs(struct chain_topology *topo, struct block *b)
{
	...
	for (i = 0; i < tal_count(b->full_txs); i++) {
		const struct xec_tx *tx = b->full_txs[i];
		struct xec_txid txid;
		size_t j;
		bool is_coinbase = i == 0;

		/* Tell them if it spends a txo we care about. */
		for (j = 0; j < tx->wtx->num_inputs; j++) {
			struct xec_outpoint out;
			struct txowatch *txo;
			xec_tx_input_get_txid(tx, j, &out.txid);
		...
		owned = AMOUNT_SAT(0);
		txid = b->txids[i];
		if (txfilter_match(topo->xecd->ld->owned_txfilter, tx)) {
			wallet_extract_owned_outputs(topo->xecd->ld->wallet,
...
struct txs_to_broadcast {
	...
	size_t cursor;
	/* These are hex encoded already, for xecd_sendrawtx */
	const char **txs;
...
static void broadcast_remainder(struct xecd *xecd,
...
/* FIXME: This is dumb.  We can group txs and avoid bothering xecd
...
static void rebroadcast_txs(struct chain_topology *topo)
{
	...
	for (otx = outgoing_tx_map_first(topo->outgoing_txs, &it); otx;
		...
		tal_arr_expand(&txs->txs, fmt_xec_tx(txs->txs, otx->tx));
	...
	txs->cursor = (size_t)-1;
	broadcast_remainder(topo->xecd, true, "", txs);
}
...
static void broadcast_done(struct xecd *xecd,
			   bool success, const char *msg,
			   struct outgoing_tx *otx)
{
	/* Channel gone?  Stop. */
	...
	if (otx->finished) {
		...
		tal_free(otx);
	} else if (we_broadcast(xecd->ld->topology, &otx->txid)) {
		log_debug(
		    xecd->ld->topology->log,
		    "Not adding %s to list of outgoing transactions, already "
		    "present",
		    type_to_string(tmpctx, struct xec_txid, &otx->txid));
		tal_free(otx);
	} else {
		/* For continual rebroadcasting, until channel freed. */
		tal_steal(otx->channel, otx);
		outgoing_tx_map_add(xecd->ld->topology->outgoing_txs, otx);
		tal_add_destructor2(otx, destroy_outgoing_tx, xecd->ld->topology);
	}
}

void broadcast_tx_(struct chain_topology *topo,
		   struct channel *channel, const struct xec_tx *tx,
		   const char *cmd_id, bool allowhighfees, u32 minblock,
		   void (*finished)(struct channel *channel,
				    bool success,
				    const char *err),
		   bool (*refresh)(struct channel *channel,
				   const struct xec_tx **tx,
				   void *arg),
		   void *refresh_arg)
{
	...
	otx->channel = channel;
	xec_txid(tx, &otx->txid);
	otx->tx = clone_xec_tx(otx, tx);
	...
	xecd_sendrawtx(topo->xecd, otx->cmd_id,
			   fmt_xec_tx(tmpctx, otx->tx),
			   allowhighfees,
			   broadcast_done, otx);
}
						  ...
						  unsigned int depth)
{
	/* Sanity check. */
	if (tx != NULL) {
		struct xec_txid txid2;

		xec_txid(tx, &txid2);
		if (!xec_txid_eq(txid, &txid2)) {
...
static void start_fee_estimate(struct chain_topology *topo)
{
	...
	xecd_estimate_fees(topo->xecd, update_feerates);
}
					    ...
					    const jsmntok_t *params)
{
	...
	if (missing)
		json_add_string(response, "warning_missing_feerates",
				"Some fee estimates unavailable: xecd startup?");
...
{
	if (!xec_blkid_eq(&topo->tip->blkid, &topo->prev_tip)) {
		/* Tell watch code to re-evaluate all txs. */
		watch_topology_changed(topo);

		/* Tell lightningd about new block. */
		notify_new_block(topo->xecd->ld, topo->tip->height);

		/* Maybe need to rebroadcast. */
		rebroadcast_txs(topo);

		/* We've processed these UTXOs */
		db_set_intvar(topo->xecd->ld->wallet->db,
	...
	/* If xecd is synced, we're now synced. */
	if (topo->xecd->synced && !topology_synced(topo)) {
				...
				u32 tx_blockheight)
{
	...
	if (!utxo) {
		log_broken(ld->log, "No record of utxo %s",
			   type_to_string(tmpctx, struct xec_outpoint,
					  outpoint));
		return;
	}
...
static void topo_update_spends(struct chain_topology *topo, struct block *b)
{
	...
	for (size_t i = 0; i < tal_count(b->full_txs); i++) {
		...
		for (size_t j = 0; j < tx->wtx->num_inputs; j++) {
			...
			xec_tx_input_get_outpoint(tx, j, &outpoint);
	...
	gossipd_notify_spends(topo->xecd->ld, b->height, spent_scids);
}
...
static void topo_add_utxos(struct chain_topology *topo, struct block *b)
{
	for (size_t i = 0; i < tal_count(b->full_txs); i++) {
		...
		for (outpoint.n = 0;
			...
			const u8 *script = xec_tx_output_get_script(tmpctx, tx, outpoint.n);
			struct amount_asset amt = xec_tx_output_get_amount(tx, outpoint.n);
			       ...
			       unsigned int height)
{
	...
	xec_block_blkid(blk, &b->blkid);
	log_debug(topo->log, "Adding block %u: %s",
		  height,
		  type_to_string(tmpctx, struct xec_blkid, &b->blkid));
...
static void remove_tip(struct chain_topology *topo)
{
	...
	gossipd_notify_spends(topo->xecd->ld, b->height, removed_scids);
	tal_free(b);
}

static void get_new_block(struct xecd *xecd,
			  ...
			  struct chain_topology *topo)
{
	...
	if (!xec_blkid_eq(&topo->tip->blkid, &blk->hdr.prev_hash))
		remove_tip(topo);
	else {
...
static void try_extend_tip(struct chain_topology *topo)
{
	...
	if (topo->stopping)
		return;
	xecd_getrawblockbyheight(topo->xecd, topo->tip->height + 1,
				     get_new_block, topo);
}

static void init_topo(struct xecd *xecd UNUSED,
		      struct xec_blkid *blkid UNUSED,
		      struct xec_block *blk,
		      struct chain_topology *topo)
{
...
u32 default_locktime(const struct chain_topology *topo)
{
	 ...
	 *   general-purpose transactions which looks like xecd:
...
struct chain_topology *new_topology(struct lightningd *ld, struct log *log)
{
	...
	topo->log = log;
	topo->xecd = new_xecd(topo, ld, log);
...
{
	/* If xecd's current blockheight is below the requested
	...
	if (blockcount < topo->max_blockheight) {
		...
		if (topo->max_blockheight == UINT32_MAX) {
			...
			if (blockcount < topo->xecd->ld->config.rescan)
				topo->max_blockheight = 0;
			else
				topo->max_blockheight = blockcount - topo->xecd->ld->config.rescan;
		} else
			fatal("xecd has gone backwards from %u to %u blocks!",
...
static void
check_chain(struct xecd *xecd, const char *chain,
	    ...
	    const bool first_call, struct chain_topology *topo)
{
	if (!streq(chain, chainparams->bip70_name))
		fatal("Wrong network! Our xec backend is running on '%s',"
		      " but we expect '%s'.", chain, chainparams->bip70_name);

	topo->headercount = headercount;

	if (first_call) {
		/* Has the xec backend gone backward ? */
		check_blockcount(topo, blockcount);
		/* Get up to speed with topology. */
		xecd_getrawblockbyheight(topo->xecd, topo->max_blockheight,
					     init_topo, topo);
	}
	...
	if (ibd) {
		...
		if (!first_call)
			log_unusual(xecd->log,
				    "xec backend now synced.");
		xecd->synced = true;
		return;
	}

	assert(!xecd->checkchain_timer);
	xecd->checkchain_timer
		= new_reltimer(xecd->ld->timers, xecd,
			       /* Be 4x more aggressive in this case. */
			       time_divide(time_from_sec(xecd->ld->topology
							 ->poll_seconds), 4),
			       retry_check_chain, xecd->ld->topology);
}

static void retry_check_chain(struct chain_topology *topo)
{
	topo->xecd->checkchain_timer = NULL;
	if (topo->stopping)
		return;
	xecd_getchaininfo(topo->xecd, false, topo->max_blockheight, check_chain, topo);
}
		    ...
		    u32 min_blockheight, u32 max_blockheight)
{
	...
	/* This waits for xecd. */
	xecd_check_commands(topo->xecd);

	/* For testing.. */
	log_debug(topo->ld->log, "All xec plugin commands registered");

	/* Sanity checks, then topology initialization. */
	topo->xecd->checkchain_timer = NULL;
	xecd_getchaininfo(topo->xecd, true, topo->max_blockheight, check_chain, topo);
...
void stop_topology(struct chain_topology *topo)
{
	...
	tal_free(topo->xecd->checkchain_timer);
