...
resolve_one_close_command(struct close_command *cc, bool cooperative)
{
	...
	if (!invalid_last_tx(cc->channel->last_tx)) {
		...
		xec_txid(cc->channel->last_tx, &txid);
				     ...
				     const struct xec_tx *tx)
{
	...
	for (size_t i = 0; i < tx->wtx->num_outputs; i++) {
		amt = xec_tx_output_get_amount(tx, i);
		oscript = xec_tx_output_get_script(NULL, tx, i);
		...
		if (!amount_sat_sub(&fee, fee, amount_asset_to_sat(&amt)))
			fatal("Tx spends more than input %s? %s",
			      ...
			      type_to_string(tmpctx, struct xec_tx, tx));
	}
	return fee;
}
				      ...
				      const struct xec_tx *tx)
{
	...
	weight = xec_tx_weight(tx) + xec_tx_2of2_input_witness_weight();
					    ...
					    const u8 *msg)
{
	...
	funding_wscript = xec_redeem_2of2(tmpctx,
	...
	xec_txid(channel->last_tx, &tx_id);
...
static struct command_result *param_outpoint(struct command *cmd,
					     ...
					     const char *buffer,
					     const jsmntok_t *tok,
					     struct xec_outpoint **outp)
{
	*outp = tal(cmd, struct xec_outpoint);
					 ...
					 const jsmntok_t *params)
{
	...
	struct xec_outpoint *wrong_funding;
	u32 *feerate_range;
	char* end;
	bool anysegwit;
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt_def("unilateraltimeout", param_number, &timeout,
			     48 * 3600),
		   p_opt("destination", param_xec_address, &close_to_script),
