 ...
 * between them, handle the JSON RPC requests, xecd, the database
...
int main(int argc, char *argv[])
{
	 ...
	 * similarly to what xec Core does to ports by default. */
	 ...
	 * xec wallet (though it's that too).  It also stores channel
	 * states, invoices, payments, blocks and xec transactions. */
	 ...
	 * an uninitialized wallet and that we should start off of xecd's
	 * current height */
	 ...
	 * talk to xecd, so does its own db transactions. */
	 ...
	 * transaction, in case xecd lost it. */
	db_begin_transaction(ld->wallet->db);
	 ...
	 *  can start the poll loop which queries xecd for new blocks. */
