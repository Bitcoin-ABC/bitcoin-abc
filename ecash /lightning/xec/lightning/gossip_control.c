...
static void got_txout(struct xecd *xecd,
...
static void got_filteredblock(struct xecd *xecd,
		      const struct filteredblock *fb,
		      struct short_channel_id *scid)
{
	struct filteredblock_outpoint *fbo = NULL, *o;
	struct xec_tx_output txo;

	/* If we failed to the filtered block we report the failure to
	 * got_txout. */
	if (fb == NULL)
		return got_txout(xecd, NULL, scid);

	/* Only fill in blocks that we are not going to scan later. */
	if (xecd->ld->topology->max_blockheight > fb->height)
		wallet_filteredblock_add(xecd->ld->wallet, fb);
	...
	if (fbo) {
		...
		got_txout(xecd, &txo, scid);
	} else
		got_txout(xecd, NULL, scid);
}
...
static void get_txout(struct subd *gossip, const u8 *msg)
{
	...
	if (op) {
		...
		xecd_getfilteredblock(topo->xecd, short_channel_id_blocknum(scid), got_filteredblock, scid);
	}
}
