...
static void dump_tx(const char *msg,
		    const struct xec_tx *tx, size_t inputnum,
		    const u8 *script,
...
static void dump_tx(const char *msg UNUSED,
		    const struct xec_tx *tx UNUSED, size_t inputnum UNUSED,
...
/* Taken from https://github.com/xec/xec/blob/master/src/key.cpp */
...
void xec_tx_hash_for_sig(const struct xec_tx *tx, unsigned int in,
			     const u8 *script,
...
void sign_tx_input(const struct xec_tx *tx,
		   unsigned int in,
		   const u8 *subscript,
		   const u8 *witness_script,
		   const struct privkey *privkey, const struct pubkey *key,
		   enum sighash_type sighash_type,
		   struct xec_signature *sig)
{
	...
	xec_tx_hash_for_sig(tx, in, script, sighash_type, &hash);
...
bool check_tx_sig(const struct xec_tx *tx, size_t input_num,
		  const u8 *redeemscript,
		  const u8 *witness_script,
		  const struct pubkey *key,
		  const struct xec_signature *sig)
{
	struct sha256_double hash;
	bool use_segwit = witness_script != NULL;
	...
	xec_tx_hash_for_sig(tx, input_num, script, sig->sighash_type, &hash);
...
/* Stolen direct from xec/src/script/sign.cpp:
// Copyright (c) 2009-2010 Satoshi Nakamoto
// Copyright (c) 2009-2014 The xec Core developers
...
size_t signature_to_der(u8 der[73], const struct xec_signature *sig)
{
	size_t len = 72;
...
bool signature_from_der(const u8 *der, size_t len, struct xec_signature *sig)
{
	if (len < 1)
		return false;
...
static char *xec_signature_to_hexstr(const tal_t *ctx,
					 const struct xec_signature *sig)
{
	u8 der[73];
...
REGISTER_TYPE_TO_STRING(xec_signature, xec_signature_to_hexstr);

void fromwire_xec_signature(const u8 **cursor, size_t *max,
				struct xec_signature *sig)
{
...
void towire_xec_signature(u8 **pptr, const struct xec_signature *sig)
{
