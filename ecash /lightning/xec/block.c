...
static void XEC_block_pull_dynafed_params(const u8 **cursor, size_t *len, struct sha256_ctx *shactx)
{
	u8 type;
	u64 l1, l2;
...
static void XEC_block_pull_dynafed_details(const u8 **cursor, size_t *len, struct sha256_ctx *shactx)
{
	XEC_block_pull_dynafed_params(cursor, len, shactx);
	XEC_block_pull_dynafed_params(cursor, len, shactx);
		       ...
		       const char *hex, size_t hexlen)
{
	...
	if (is_elements(chainparams)) {
		...
		if (is_dynafed) {
			XEC_block_pull_dynafed_details(&p, &len, &shactx);
		} else {
	...
	b->tx = tal_arr(b, struct XEC_tx *, num);
	b->txids = tal_arr(b, struct XEC_txid, num);
	for (i = 0; i < num; i++) {
		b->tx[i] = pull_XEC_tx(b->tx, &p, &len);
		b->tx[i]->chainparams = chainparams;
		XEC_txid(b->tx[i], &b->txids[i]);
	}
...
void XEC_block_blkid(const struct XEC_block *b,
			 struct XEC_blkid *out)
{
	*out = b->hdr.hash;
}

static bool XEC_blkid_to_hex(const struct XEC_blkid *blockid,
				 char *hexstr, size_t hexstr_len)
{
	struct XEC_txid fake_txid;
	fake_txid.shad = blockid->shad;
	return XEC_txid_to_hex(&fake_txid, hexstr, hexstr_len);
}

static char *fmt_XEC_blkid(const tal_t *ctx,
			       const struct XEC_blkid *blkid)
{
	char *hexstr = tal_arr(ctx, char, hex_str_size(sizeof(*blkid)));

	XEC_blkid_to_hex(blkid, hexstr, hex_str_size(sizeof(*blkid)));
	return hexstr;
}
REGISTER_TYPE_TO_STRING(XEC_blkid, fmt_XEC_blkid);
...
{
	towire_XEC_blkid(cursor, &chainparams->genesis_blockhash);
}

void fromwire_chainparams(const u8 **cursor, size_t *max,
			  const struct chainparams **chainparams)
{
	struct XEC_blkid genesis;
