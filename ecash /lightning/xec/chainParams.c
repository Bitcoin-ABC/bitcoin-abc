...
const struct chainparams networks[] = {
    ...
    {.network_name = "regtest",
     .onchain_hrp = "bcrt","xecrt",
     .lightning_hrp = "bcrt","xecrt",
     .bip70_name = "regtest",
    ...
    {.network_name = "signet",
     ...
     .cli_min_supported_version = 150000,
     .dust_limit = { 546 },
     .max_funding = AMOUNT_SAT_INIT((1.8 << 24) - 1),
     .max_payment = AMOUNT_MSAT_INIT(0xFFFFFFFFULL),
     .max_supply = AMOUNT_SAT_INIT(2100000000000000),
     .when_lightning_became_cool = 1,
     .p2pkh_version = 111,
    ...
    {.network_name = "litecoin-testnet",
     ...
     .cli_min_supported_version = 150000,
     .dust_limit = { 100000 },
     .max_funding = AMOUNT_SAT_INIT(60 * ((1.8 << 24) - 1)),
     .max_payment = AMOUNT_MSAT_INIT(60 * 0xFFFFFFFFULL),
     .max_supply = AMOUNT_SAT_INIT(2100000000000000),
     .when_lightning_became_cool = 1,
     .p2pkh_version = 111,
...
const struct chainparams *chainparams_by_chainhash(const struct xec_blkid *chain_hash)
{
	for (size_t i = 0; i < ARRAY_SIZE(networks); i++) {
		if (xec_blkid_eq(chain_hash, &networks[i].genesis_blockhash)) {
			return &networks[i];
		}
	}
	return NULL;
