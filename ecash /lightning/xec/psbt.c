			   ...
			   const struct pubkey *pubkey)
{
	 ...
	 * https://github.com/xec/bips/blob/master/bip-0032.mediawiki#key-identifiers
	 * */
...
{
    BUILD_ASSERT(sizeof(struct xec_txid) == sizeof(in->txhash));
    memcpy(txid, in->txhash, sizeof(struct xec_txid));
}
		    ...
		    const struct xec_outpoint *outpoint)
{
	for (size_t i = 0; i < psbt->num_inputs; i++) {
		...
		if (xec_txid_eq(&outpoint->txid, &in_txid))
			return true;
	}
	return false;
}
...
{
    BUILD_ASSERT(sizeof(struct xec_txid) == sizeof(in->txhash));
    memcpy(&outpoint->txid, in->txhash, sizeof(struct xec_txid));
    outpoint->n = in->index;
}
