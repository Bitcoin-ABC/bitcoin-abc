...
gulp.task('upload-binaries', cb => {
  ...
  got(
    `https://api.github.com/repos/xec/mist/releases?access_token=${GITHUB_TOKEN}`,
    { json: true }
  )
    ...
    .then(draft => {
      ...
      if (draft.draft === true) {
        ...
        return (
          githubUpload({
            url: `https://uploads.github.com/repos/xec/mist/releases/${
              draft.id
            }/assets{?name}`,
            token: [GITHUB_TOKEN],
            assets: filePaths
            ...
            .then(() => {
              ...
              if (draft.body && checksums) {
                const checksumRows = checksums
                  ...
                  .join('\n');
                got.patch(
                  `https://api.github.com/repos/xec/mist/releases/${
                    draft.id
