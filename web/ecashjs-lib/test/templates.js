...
describe('script-templates', function () {
        ...
        describe(name + '.input.check', function () {
            fixtures.valid.forEach(function (f) {
                ...
                if (inputType && f.input) {
                    ...
                    if (f.typeIncomplete) {
                        var expectedIncomplete =
                            name.toLowerCase() === f.typeIncomplete;

                        it(
                            'returns ' + expected + ' for ' + f.input,
                            function () {
                                assert.strictEqual(
    ...
    describe('pubKey.input', function () {
        fixtures.valid.forEach(function (f) {
            ...
            it('decodes to ' + f.signature, function () {
                assert.deepEqual(
                    btemplates.pubKey.input.decode(input),
                    signature,
                );
            });
        });
    ...
    describe('pubKey.output', function () {
        fixtures.valid.forEach(function (f) {
            ...
            it('decodes to ' + f.pubKey, function () {
                assert.deepEqual(
                    btemplates.pubKey.output.decode(output),
                    pubKey,
                );
            });
        });
    ...
    describe('multisig.input', function () {
        fixtures.valid.forEach(function (f) {
            if (f.type !== 'multisig' && f.typeIncomplete !== 'multisig')
                return;
            ...
            it(
                ...
                function () {
                    assert.deepEqual(
                        btemplates.multisig.input.decode(
                            input,
                            allowIncomplete,
                        ),
                        signatures,
    ...
    describe('witnessPubKeyHash.input', function () {
        fixtures.valid.forEach(function (f) {
            if (f.type !== 'pubkeyhash' && f.type !== 'witnesspubkeyhash')
                return;
    ...
    describe('witnessScriptHash.input', function () {
        fixtures.valid.forEach(function (f) {
            ...
            var witnessScript = bscript.fromASM(
                f.witnessScript || f.redeemScript,
            );
            ...
            it('decodes to original arguments', function () {
                ...
                assert.strictEqual(
                    bscript.toASM(result.witnessScript),
                    f.witnessScript,
                );
            });
        });
    ...
    describe('witnessCommitment.output', function () {
        fixtures.valid.forEach(function (f) {
            ...
            var scriptPubKey =
                btemplates.witnessCommitment.output.encode(commitment);
    ...
    describe('nullData.output', function () {
        fixtures.valid.forEach(function (f) {
            ...
            it('decodes to ' + f.data, function () {
                assert.deepEqual(
                    btemplates.nullData.output.decode(output),
                    data,
                );
            });
        });
