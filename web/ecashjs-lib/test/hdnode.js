...
fixtures.valid.forEach(function (f) {
    ...
    validAll = validAll.concat(
        addNetwork(f.master),
        f.children.map(addNetwork),
    );
});
    ...
    describe('getFingerprint', function () {
        validAll.forEach(function (f) {
            it('returns the fingerprint for ' + f.fingerprint, function () {
                ...
                assert.strictEqual(
                    hd.getFingerprint().toString('hex'),
                    f.fingerprint,
                );
            });
        });
    ...
    describe('derive', function () {
        ...
        fixtures.valid.forEach(function (f) {
            ...
            f.children.forEach(function (c) {
                it(
                    c.path + ' from ' + f.master.fingerprint + ' by path',
                    function () {
            ...
            f.children.forEach(function (c, i) {
                ...
                f.children.slice(i + 1).forEach(function (cc) {
                    it(
                        cc.path + ' from ' + c.fingerprint + ' by path',
                        function () {
                            var ipath = cc.path
                                .slice(2)
                                .split('/')
