...
function rng() {
    return Buffer.from(
        'YT8dAtK4d16A3P1z+TpwB2jJ4aFH3g9M1EioIBkLEV4=',
        'base64',
    );
}
...
describe('bitcoinjs-lib (transactions)', function () {
    ...
    it('can verify Transaction signatures', function () {
        ...
        tx.ins.forEach(function (input, i) {
            ...
            var scriptSig = bitcoin.script.pubKeyHash.input.decode(
                input.script,
            );
            var ss = bitcoin.ECSignature.parseScriptSignature(
                scriptSig.signature,
            );
            var hash = tx.hashForSignature(i, prevOutScript, ss.hashType);

            assert.strictEqual(
                scriptSig.pubKey.toString('hex'),
