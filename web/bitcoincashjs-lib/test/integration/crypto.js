...
describe('bitcoinjs-lib (crypto)', function () {
    it('can recover a private key from duplicate R values', function (done) {
        ...
        mainnet.transactions.get(txIds, function (err, results) {
            ...
            inputs.forEach(function (input) {
                ...
                var prevOutTxId = Buffer.from(transaction.ins[input.vout].hash)
                    .reverse()
                    .toString('hex');
                var prevVout = transaction.ins[input.vout].index;

                tasks.push(function (callback) {
                    mainnet.transactions.get(
                        prevOutTxId,
                        function (err, result) {
                            if (err) return callback(err);

                            var prevOut = bitcoin.Transaction.fromHex(
                                result.txHex,
                            );
                            var prevOutScript = prevOut.outs[prevVout].script;

                            var scriptSignature =
                                bitcoin.ECSignature.parseScriptSignature(
                                    scriptChunks[0],
                                );
                            var publicKey = bitcoin.ECPair.fromPublicKeyBuffer(
                                scriptChunks[1],
                            );

                            var m = transaction.hashForSignature(
                                input.vout,
                                prevOutScript,
                                scriptSignature.hashType,
                            );
                            assert(
                                publicKey.verify(m, scriptSignature.signature),
                                'Invalid m',
                            );
    ...
    it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
        function recoverParent(master, child) {
            assert(
                !master.keyPair.d,
                'You already have the parent private key',
            );
