...
constexpr size_t NATPMP_GETEXTERNAL_RESPONSE_SIZE =
    NATPMP_RESPONSE_HDR_SIZE + 4;
...
//!  Offset of result code in packets. Result codes are 16 bit in NAT-PMP
//!  instead of 8 bit in PCP.
...
//! PCP uses a fixed server port number (RFC6887 section 19.1). Shared with
//! NAT-PMP.
...
// Header offsets shared between request and responses (RFC6887 7.1, 7.2),
// relative to start of packet.
...
// MAP request/response offsets (RFC6887 11.1), relative to start of
...
//!  Suggested external port (request), assigned external port (response) (2
//!  bytes).
constexpr size_t PCP_MAP_EXTERNAL_PORT_OFS = 18;
//!  Suggested external IP (request), assigned external IP (response) (16
//!  bytes).
...
//! Mapping of PCP result code to string (RFC6887 7.4). Result codes <=2 match
//! NAT-PMP.
const std::map<uint8_t, std::string> PCP_RESULT_STR{
    {0, "SUCCESS"},           {1, "UNSUPP_VERSION"},
    {2, "NOT_AUTHORIZED"},    {3, "MALFORMED_REQUEST"},
    {4, "UNSUPP_OPCODE"},     {5, "UNSUPP_OPTION"},
    {6, "MALFORMED_OPTION"},  {7, "NETWORK_FAILURE"},
    {8, "NO_RESOURCES"},      {9, "UNSUPP_PROTOCOL"},
    {10, "USER_EX_QUOTA"},    {11, "CANNOT_PROVIDE_EXTERNAL"},
    {12, "ADDRESS_MISMATCH"}, {13, "EXCESSIVE_REMOTE_PEER"},
};
...
std::string PCPResultString(uint8_t result_code) {
    ...
    return strprintf("%s (code %d)",
                     result_i == PCP_RESULT_STR.end() ? "(unknown)"
                                                      : result_i->second,
                     result_code);
}

//! Wrap address in IPv6 according to RFC6887. wrapped_addr needs to be able to
//! store 16 bytes.
[[nodiscard]] bool PCPWrapAddress(Span<uint8_t> wrapped_addr,
                                  const CNetAddr &addr) {
    Assume(wrapped_addr.size() == ADDR_IPV6_SIZE);
    if (addr.IsIPv4()) {
        struct in_addr addr4;
        if (!addr.GetInAddr(&addr4)) {
            return false;
        }
        // Section 5: "When the address field holds an IPv4 address, an
        // IPv4-mapped IPv6 address [RFC4291] is used (::ffff:0:0/96)."
        std::memcpy(wrapped_addr.data(), IPV4_IN_IPV6_PREFIX.data(),
                    IPV4_IN_IPV6_PREFIX.size());
        std::memcpy(wrapped_addr.data() + IPV4_IN_IPV6_PREFIX.size(), &addr4,
                    ADDR_IPV4_SIZE);
        return true;
    }
    if (addr.IsIPv6()) {
        struct in6_addr addr6;
        if (!addr.GetIn6Addr(&addr6)) {
            return false;
        }
...
CNetAddr PCPUnwrapAddress(Span<const uint8_t> wrapped_addr) {
    ...
    if (util::HasPrefix(wrapped_addr, IPV4_IN_IPV6_PREFIX)) {
        struct in_addr addr4;
        std::memcpy(&addr4, wrapped_addr.data() + IPV4_IN_IPV6_PREFIX.size(),
                    ADDR_IPV4_SIZE);
...
PCPSendRecv(Sock &sock, const std::string &protocol,
    ...
    for (int ntry = 0; !got_response && ntry < num_tries; ++ntry) {
        ...
        // Wait for response(s) until we get a valid response, a network error,
        // or time out.
        auto cur_time = time_point_cast<milliseconds>(steady_clock::now());
        auto deadline = cur_time + timeout_per_try;
        while ((cur_time = time_point_cast<milliseconds>(steady_clock::now())) <
               deadline) {
            ...
            if (check_packet(Span<uint8_t>(response, recvsz))) {
                // Got expected response, break from receive loop as well as
                // from retry loop.
                got_response = true;
                break;
            }
...
NATPMPRequestPortMap(const CNetAddr &gateway, uint16_t port, uint32_t lifetime,
    ...
    if (!CService(gateway, PCP_SERVER_PORT)
             .GetSockAddr((struct sockaddr *)&dest_addr, &dest_addrlen)) {
    ...
    if (!sock) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "natpmp: Could not create UDP socket: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (sock->Connect((struct sockaddr *)&dest_addr, dest_addrlen) != 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "natpmp: Could not connect to gateway: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }

    // Use getsockname to get the address toward the default gateway (the
    // internal address).
    struct sockaddr_in internal;
    socklen_t internal_addrlen = sizeof(struct sockaddr_in);
    if (sock->GetSockName((struct sockaddr *)&internal, &internal_addrlen) !=
        0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "natpmp: Could not get sock name: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    auto recv_res = PCPSendRecv(
        *sock, "natpmp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            if (response.size() < NATPMP_GETEXTERNAL_RESPONSE_SIZE) {
                LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                              "natpmp: Response too small\n");
                // Wasn't response to what we expected, try receiving next
                // packet.
                return false;
            }
            if (response[NATPMP_HDR_VERSION_OFS] != NATPMP_VERSION ||
                response[NATPMP_HDR_OP_OFS] !=
                    (NATPMP_RESPONSE | NATPMP_OP_GETEXTERNAL)) {
                LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
    ...
    if (recv_res) {
        ...
        std::memcpy(&external_addr,
                    response.data() + NATPMP_GETEXTERNAL_RESPONSE_IP_OFS,
                    ADDR_IPV4_SIZE);
    } else {
        return MappingError::NETWORK_ERROR;
    }
        ...
        *sock, "natpmp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            ...
            if (response[0] != NATPMP_VERSION ||
                response[1] != (NATPMP_RESPONSE | NATPMP_OP_MAP_TCP)) {
                LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                              "natpmp: Response to wrong command\n");
                // Wasn't response to what we expected, try receiving next
                // packet.
            ...
            uint16_t internal_port = ReadBE16(
                response.data() + NATPMP_MAP_RESPONSE_INTERNAL_PORT_OFS);
    ...
    if (recv_res) {
        ...
        uint32_t lifetime_ret =
            ReadBE32(response.data() + NATPMP_MAP_RESPONSE_LIFETIME_OFS);
        uint16_t external_port =
            ReadBE16(response.data() + NATPMP_MAP_RESPONSE_EXTERNAL_PORT_OFS);
        return MappingResult(NATPMP_VERSION, CService(internal.sin_addr, port),
                             CService(external_addr, external_port),
                             lifetime_ret);
    }
    return MappingError::NETWORK_ERROR;
}
...
PCPRequestPortMap(const PCPMappingNonce &nonce, const CNetAddr &gateway,
    ...
    if (!CService(bind, 0).GetSockAddr((struct sockaddr *)&bind_addr,
                                       &bind_addrlen)) {
        return MappingError::NETWORK_ERROR;
    }
    if (dest_addr.ss_family != bind_addr.ss_family) {
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (!sock) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "pcp: Could not create UDP socket: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }

    // Make sure that we send from requested destination address, anything else
    // will be rejected by a security-conscious router.
    if (sock->Bind((struct sockaddr *)&bind_addr, bind_addrlen) != 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "pcp: Could not bind to address: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    ...
    if (sock->Connect((struct sockaddr *)&dest_addr, dest_addrlen) != 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "pcp: Could not connect to gateway: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }

    // Use getsockname to get the address toward the default gateway (the
    // internal address), in case we don't know what address to map (this is
    // only needed if bind is INADDR_ANY, but it doesn't hurt as an extra
    // check).
    struct sockaddr_storage internal_addr;
    socklen_t internal_addrlen = sizeof(struct sockaddr_storage);
    if (sock->GetSockName((struct sockaddr *)&internal_addr,
                          &internal_addrlen) != 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                      "pcp: Could not get sock name: %s\n",
                      NetworkErrorString(WSAGetLastError()));
        return MappingError::NETWORK_ERROR;
    }
    CService internal;
    if (!internal.SetSockAddr((struct sockaddr *)&internal_addr,
                              internal_addrlen)) {
        return MappingError::NETWORK_ERROR;
    }
    LogPrintLevel(BCLog::NET, BCLog::Level::Debug,
                  "pcp: Internal address after connect: %s\n",
                  internal.ToStringAddr());

    // Build request packet. Make sure the packet is zeroed so that reserved
    // fields are zero as required by the spec (and not potentially leak data).
    // Make sure there's space for the request header and MAP specific request
    // data.
    ...
    if (!PCPWrapAddress(
            Span(request).subspan(ofs + PCP_REQUEST_HDR_IP_OFS, ADDR_IPV6_SIZE),
            internal)) {
        return MappingError::NETWORK_ERROR;
    }
    ...
    // correlate requests and responses, and used to authenticate changes to the
    // mapping).
    std::memcpy(request.data() + ofs + PCP_MAP_NONCE_OFS, nonce.data(),
                PCP_MAP_NONCE_SIZE);
    ...
    if (!PCPWrapAddress(Span(request).subspan(ofs + PCP_MAP_EXTERNAL_IP_OFS,
                                              ADDR_IPV6_SIZE),
                        bind)) {
        return MappingError::NETWORK_ERROR;
    }
    ...
    auto recv_res = PCPSendRecv(
        *sock, "pcp", request, num_tries, timeout_per_try,
        [&](const Span<const uint8_t> response) -> bool {
            // Unsupported version according to RFC6887 appendix A and RFC6886
            // section 3.5, can fall back to NAT-PMP.
            if (response.size() == NATPMP_RESPONSE_HDR_SIZE &&
                response[PCP_HDR_VERSION_OFS] == NATPMP_VERSION &&
                response[PCP_RESPONSE_HDR_RESULT_OFS] ==
                    NATPMP_RESULT_UNSUPP_VERSION) {
                is_natpmp = true;
                // Let it through to caller.
                return true;
            }
            ...
            if (response[PCP_HDR_VERSION_OFS] != PCP_VERSION ||
                response[PCP_HDR_OP_OFS] != (PCP_RESPONSE | PCP_OP_MAP)) {
                LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
            ...
            if (!std::ranges::equal(
                    response.subspan(PCP_HDR_SIZE + PCP_MAP_NONCE_OFS,
                                     PCP_MAP_NONCE_SIZE),
                    nonce)) {
                LogPrintLevel(BCLog::NET, BCLog::Level::Warning,
                              "pcp: Mapping nonce mismatch\n");
                // Wasn't response to what we expected, try receiving next
                // packet.
                return false;
            }
    ...
    return MappingResult(PCP_VERSION, CService(internal, port),
...
std::string MappingResult::ToString() {
    ...
    return strprintf(
        ...
        external.ToStringAddrPort(), internal.ToStringAddrPort(), lifetime);
}
