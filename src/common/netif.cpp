...
//! If a length is not given, it is taken to be sizeof(struct sockaddr_*) for
//! the family.
std::optional<CNetAddr> FromSockAddr(const struct sockaddr *addr,
                                     std::optional<socklen_t> sa_len_opt) {
    socklen_t sa_len = 0;
...
// running it requires loading a special kernel module, otherwise
// socket(AF_NETLINK,...) will fail, so we skip that.
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family) {
    ...
    struct {
        /// Request header.
        nlmsghdr hdr;
        /// Request data, a "route message".
        rtmsg data;
        /// One attribute, conveying the route destination address.
        nlattr dst_hdr;
        /// Route destination address. To query the default route we use
        /// 0.0.0.0/0 or [::]/0. For IPv4 the first 4 bytes are used.
        char dst_data[16];
    } request{};
    ...
    request.hdr.nlmsg_len =
        NLMSG_LENGTH(sizeof(rtmsg) + sizeof(nlattr) + dst_data_len);
    // Sequence number, used to match which reply is to which request.
    // Irrelevant for us because we send just one request.
    request.hdr.nlmsg_seq = 0;
    request.data.rtm_family = family;
    // Prefix length.
    request.data.rtm_dst_len = 0;
#ifdef __FreeBSD__
    ...
    if (sock->Send(&request, request.hdr.nlmsg_len, 0) !=
        static_cast<ssize_t>(request.hdr.nlmsg_len)) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "send() to netlink socket: %s\n",
                      NetworkErrorString(errno));
        return std::nullopt;
    }
...
// Suppress a false positive clang warning (NLMSG_NEXT uses NLMSG_ALIGN
// to force alignment)
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wcast-align"
#endif
        for (nlmsghdr *hdr = (nlmsghdr *)response; NLMSG_OK(hdr, recv_result);
...
#pragma clang diagnostic pop
            ...
            int scope_id = 0;
// Suppress a false positive clang warning (RTM_RTA and RTA_NEXT use NLMSG_ALIGN
// and RTA_ALIGN to force alignment)
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wcast-align"
#endif
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family) {
    ...
    // Pass empty destination address of the requested type (:: or 0.0.0.0) to
    ...
    if (status != NO_ERROR) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "Could not get best interface for default route: %s\n",
    ...
    status = GetBestRoute2(&interface_luid, best_if_idx, nullptr,
                           &destination_address, 0, &best_route,
                           &best_source_address);
    if (status != NO_ERROR) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "Could not get best route for default route for "
                      "interface index %d: %s\n",
    ...
    if (family == AF_INET6) {
        return CNetAddr(best_route.NextHop.Ipv6.sin6_addr,
                        best_route.InterfaceIndex);
    }
    return std::nullopt;
}
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family) {
    ...
    // The size of the available data is determined by calling sysctl() with
    // oldp=nullptr. See sysctl(3).
    size_t l = 0;
    if (sysctl(/*name=*/mib, /*namelen=*/sizeof(mib) / sizeof(int),
               /*oldp=*/nullptr, /*oldlenp=*/&l, /*newp=*/nullptr,
               /*newlen=*/0) < 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "Could not get sysctl length of routing table: %s\n",
                      SysErrorString(errno));
        return std::nullopt;
    }
    std::vector<std::byte> buf(l);
    if (sysctl(/*name=*/mib, /*namelen=*/sizeof(mib) / sizeof(int),
               /*oldp=*/buf.data(), /*oldlenp=*/&l, /*newp=*/nullptr,
               /*newlen=*/0) < 0) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "Could not get sysctl data of routing table: %s\n",
                      SysErrorString(errno));
        return std::nullopt;
    }
    // Iterate over messages (each message is a routing table entry).
    for (size_t msg_pos = 0; msg_pos < buf.size();) {
        if ((msg_pos + sizeof(rt_msghdr)) > buf.size()) {
            return std::nullopt;
        }
        const struct rt_msghdr *rt =
            (const struct rt_msghdr *)(buf.data() + msg_pos);
        const size_t next_msg_pos = msg_pos + rt->rtm_msglen;
        if (rt->rtm_msglen < sizeof(rt_msghdr) || next_msg_pos > buf.size()) {
            return std::nullopt;
        }
        // Iterate over addresses within message, get destination and gateway
        // (if present). Address data starts after header.
        size_t sa_pos = msg_pos + sizeof(struct rt_msghdr);
        std::optional<CNetAddr> dst, gateway;
        for (int i = 0; i < RTAX_MAX; i++) {
            if (rt->rtm_addrs & (1 << i)) {
                // 2 is just sa_len + sa_family, the theoretical minimum size of
                // a socket address.
                if ((sa_pos + 2) > next_msg_pos) {
                    return std::nullopt;
                }
                const struct sockaddr *sa =
                    (const struct sockaddr *)(buf.data() + sa_pos);
                if ((sa_pos + sa->sa_len) > next_msg_pos) {
                    return std::nullopt;
                }
                if (i == RTAX_DST) {
...
std::optional<CNetAddr> QueryDefaultGateway(Network network) {
    ...
    // It's possible for the default gateway to be 0.0.0.0 or ::0 on at least
    // Windows for some routing strategies. If so, return as if no default
    // gateway was found.
...
std::vector<CNetAddr> GetLocalAddresses() {
...
#ifdef WIN32
    DWORD status = 0;
    // Absolute maximum size of adapter addresses structure we're willing to
    // handle, as a precaution.
    constexpr size_t MAX_ADAPTER_ADDR_SIZE = 4 * 1000 * 1000;
    // Start with 15KB allocation as recommended in GetAdaptersAddresses
    // documentation.
    std::vector<std::byte> out_buf(15000, {});
    while (true) {
        ...
        status = GetAdaptersAddresses(
            ...
            nullptr, reinterpret_cast<PIP_ADAPTER_ADDRESSES>(out_buf.data()),
            &out_buf_len);
        if (status == ERROR_BUFFER_OVERFLOW &&
            out_buf.size() < MAX_ADAPTER_ADDR_SIZE) {
            // If status == ERROR_BUFFER_OVERFLOW, out_buf_len will contain the
            // needed size. Unfortunately, this cannot be fully relied on,
            // because another process may have added interfaces. So to avoid
            // getting stuck due to a race condition, double the buffer size at
            // least once before retrying (but only up to the maximum allowed
            // size).
            out_buf.resize(
                std::min(std::max<size_t>(out_buf_len, out_buf.size()) * 2,
                         MAX_ADAPTER_ADDR_SIZE));
        } else {
            break;
        }
    }
    ...
    if (status != NO_ERROR) {
        // This includes ERROR_NO_DATA if there are no addresses and thus
        // there's not even one PIP_ADAPTER_ADDRESSES record in the returned
        // structure.
        LogPrintLevel(BCLog::NET, BCLog::Level::Error,
                      "Could not get local adapter addresses: %s\n",
                      NetworkErrorString(status));
        return addresses;
    }

    // Iterate over network adapters.
    for (PIP_ADAPTER_ADDRESSES cur_adapter =
             reinterpret_cast<PIP_ADAPTER_ADDRESSES>(out_buf.data());
         cur_adapter != nullptr; cur_adapter = cur_adapter->Next) {
        if (cur_adapter->OperStatus != IfOperStatusUp) {
            continue;
        }
        ...
        // Iterate over unicast addresses for adapter, the only address type
        // we're interested in.
        for (PIP_ADAPTER_UNICAST_ADDRESS cur_address =
                 cur_adapter->FirstUnicastAddress;
             cur_address != nullptr; cur_address = cur_address->Next) {
            // "The IP address is a cluster address and should not be used by
            // most applications."
            if ((cur_address->Flags & IP_ADAPTER_ADDRESS_TRANSIENT) != 0) {
                continue;
            }

            if (std::optional<CNetAddr> addr =
                    FromSockAddr(cur_address->Address.lpSockaddr,
    ...
    if (getifaddrs(&myaddrs) == 0) {
        for (struct ifaddrs *ifa = myaddrs; ifa != nullptr;
            ...
            if (std::optional<CNetAddr> addr =
                    FromSockAddr(ifa->ifa_addr, std::nullopt)) {
                addresses.push_back(*addr);
            }
        }
        freeifaddrs(myaddrs);
