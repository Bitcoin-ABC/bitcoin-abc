...
namespace common {
std::optional<ConfigError> InitConfig(ArgsManager &args,
                                      SettingsAbortFn settings_abort_fn) {
    try {
        if (!CheckDataDirOption(args)) {
            return ConfigError{
                ConfigStatus::FAILED,
                strprintf(_("Specified data directory \"%s\" does not exist."),
                          args.GetArg("-datadir", ""))};
        }
        ...
        if (!args.ReadConfigFiles(error, true)) {
            return ConfigError{
                ConfigStatus::FAILED,
                strprintf(_("Error reading configuration file: %s"), error)};
        }

        // Check for chain settings (Params() calls are only valid after this
        // clause)
        ...
        if (!fs::exists(base_path)) {
            // When creating a *new* datadir, also create a "wallets"
            // subdirectory, whether or not the wallet is enabled now, so if the
            // wallet is enabled in the future, it will use the "wallets"
            // subdirectory for creating and listing wallets, rather than the
            // top-level directory where wallets could be mixed up with other
            // files. For backwards compatibility, wallet code will use the
            // "wallets" subdirectory only if it already exists, but never
            // create it itself. There is discussion in
            // https://github.com/bitcoin/bitcoin/issues/16220 about ways to
        ...
        if (fs::exists(base_config_path) &&
            ...
            const std::string config_source =
                cli_config_path.empty()
                    ? strprintf("data directory %s",
                                fs::quoted(fs::PathToString(orig_datadir_path)))
                    : strprintf("command line argument %s",
                                fs::quoted("-conf=" + cli_config_path));
            const std::string err = strprintf(
                "Data directory %1$s contains a %2$s file which is ignored, "
                "because a different configuration file "
                "%3$s from %4$s is being used instead. Possible ways to "
                "address this would be to:\n"
                "- Delete or rename the %2$s file in data directory %1$s.\n"
                "- Change datadir= or conf= options to specify one "
                "configuration file, not two, and use "
                "includeconf= to include any other configuration files.\n"
                "- Set allowignoredconf=1 option to treat this condition as a "
                "warning, not an error.",
                fs::quoted(fs::PathToString(base_path)),
                fs::quoted(BITCOIN_CONF_FILENAME),
                fs::quoted(fs::PathToString(orig_config_path)), config_source);
            if (args.GetBoolArg("-allowignoredconf", false)) {
                LogPrintf("Warning: %s\n", err);
            } else {
                return ConfigError{ConfigStatus::FAILED, Untranslated(err)};
            }
        }
        ...
        if (args.GetSettingsPath()) {
            ...
            if (!args.ReadSettingsFile(&details)) {
                const bilingual_str &message =
                    _("Settings file could not be read");
                if (!settings_abort_fn) {
                    ...
                    details
                        .clear(); // User chose to ignore the error and proceed.
                }
            }
            if (!args.WriteSettingsFile(&details)) {
                const bilingual_str &message =
                    _("Settings file could not be written");
                return ConfigError{ConfigStatus::FAILED_WRITE, message,
                                   details};
            }
        }
