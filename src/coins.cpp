...
std::optional<Coin> CCoinsViewBacked::GetCoin(const COutPoint &outpoint) const {
...
std::optional<Coin> CCoinsViewCache::GetCoin(const COutPoint &outpoint) const {
    if (auto it{FetchCoin(outpoint)};
        it != cacheCoins.end() && !it->second.coin.IsSpent()) {
...
void CCoinsViewCache::AddCoin(const COutPoint &outpoint, Coin coin,
    ...
    ++m_dirty_count;
    if (fresh) {
        CCoinsCacheEntry::SetFresh(*it, m_sentinel);
    }
    ...
    for (auto it{cursor.Begin()}; it != cursor.End();
        ...
        if (inserted) {
            if (it->second.IsFresh() && it->second.coin.IsSpent()) {
                ...
                if (it->second.IsFresh()) {
                    CCoinsCacheEntry::SetFresh(*itUs, m_sentinel);
                }
            }
        } else {
            ...
            if (itUs->second.IsFresh() && it->second.coin.IsSpent()) {
                ...
                Assume(TrySub(cachedCoinsUsage,
                              itUs->second.coin.DynamicMemoryUsage()));
                cacheCoins.erase(itUs);
            } else {
...
void CCoinsViewCache::Flush(bool reallocate_cache) {
    auto cursor{CoinsViewCacheCursor(m_dirty_count, m_sentinel, cacheCoins,
                                     /*will_erase=*/true)};
    base->BatchWrite(cursor, hashBlock);
...
static ReturnType
ExecuteBackedWrapper(Func func,
                     const std::vector<std::function<void()>> &err_callbacks) {
    try {
        return func();
...
std::optional<Coin>
CCoinsViewErrorCatcher::GetCoin(const COutPoint &outpoint) const {
    return ExecuteBackedWrapper<std::optional<Coin>>(
        [&]() { return CCoinsViewBacked::GetCoin(outpoint); }, m_err_callbacks);
}
