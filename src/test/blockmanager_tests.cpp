...
BOOST_AUTO_TEST_CASE(blockmanager_find_block_pos) {
    ...
    // blk00000.dat file the block is found at offset 8 because there is an 8
    // byte serialization header consisting of 4 magic bytes + 4 length bytes
    ...
    // https://github.com/bitcoin/bitcoin/issues/21379 does not recur 8 bytes
...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file) {
    ...
    // Write the first block; dbp=nullptr means this block doesn't already have
    // a disk location, so allocate a free location and write it there.
    FlatFilePos pos1{
        blockman.SaveBlockToDisk(block1, /*nHeight=*/1, /*dbp=*/nullptr)};

    // Write second block
    FlatFilePos pos2{
        blockman.SaveBlockToDisk(block2, /*nHeight=*/2, /*dbp=*/nullptr)};
    ...
    BOOST_CHECK(
        blockman.SaveBlockToDisk(block3, /*nHeight=*/3, /*dbp=*/&pos2) == pos2);
    // Metadata is updated...
    ...
    //   SaveBlockToDisk() did not call WriteBlockToDisk() because `FlatFilePos*
    //   dbp` was non-null
