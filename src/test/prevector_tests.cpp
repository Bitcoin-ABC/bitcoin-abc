    ...
    for (int j = 0; j < 64; j++) {
        ...
        for (int i = 0; i < 2048; i++) {
            if (insecure_randbits(2) == 0) {
                test.insert(insecure_randrange(test.size() + 1),
                            insecure_rand());
            }
            ...
            if (insecure_randbits(3) == 2) {
                int new_size = std::max<int>(
                    0, std::min<int>(
                           30, test.size() + (insecure_randrange(5)) - 2));
                test.resize(new_size);
            }
            if (insecure_randbits(3) == 3) {
                test.insert(insecure_randrange(test.size() + 1),
                            1 + insecure_randrange(2), insecure_rand());
            }
            if (insecure_randbits(3) == 4) {
                int del =
                    std::min<int>(test.size(), 1 + (insecure_randrange(2)));
            ...
            if (insecure_randbits(5) == 7) {
                ...
                test.insert_range(insecure_randrange(test.size() + 1), values,
                                  values + num);
            }
            if (insecure_randbits(5) == 8) {
                int del =
                    std::min<int>(test.size(), 1 + (insecure_randrange(4)));
                int beg = insecure_randrange(test.size() + 1 - del);
                test.erase(beg, beg + del);
            }
