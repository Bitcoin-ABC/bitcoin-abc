...
FUZZ_TARGET_INIT(socks5, initialize_socks5) {
    ...
    proxy_credentials.username =
        fuzzed_data_provider.ConsumeRandomLengthString(512);
    proxy_credentials.password =
        fuzzed_data_provider.ConsumeRandomLengthString(512);
    InterruptSocks5(fuzzed_data_provider.ConsumeBool());
    // Set FUZZED_SOCKET_FAKE_LATENCY=1 to exercise recv timeout code paths.
    // This will slow down fuzzing.
    ...
    (void)Socks5(fuzzed_data_provider.ConsumeRandomLengthString(512),
                 ...
                 fuzzed_data_provider.ConsumeBool() ? &proxy_credentials
                                                    : nullptr,
                 fuzzed_sock);
}
