...
void TestMultiplicationOverflow(FuzzedDataProvider &fuzzed_data_provider) {
    ...
    T result_builtin;
    const bool is_multiplication_overflow_builtin =
        __builtin_mul_overflow(i, j, &result_builtin);
    assert(is_multiplication_overflow_custom ==
...
FUZZ_TARGET(multiplication_overflow) {
    ...
    TestMultiplicationOverflow<uint8_t>(fuzzed_data_provider);
