...
int FuzzedSock::Bind(const sockaddr *, socklen_t) const {
    // Have a permanent error at bind_errnos[0] because when the fuzzed data is
    // exhausted SetFuzzedErrNo() will always set the global errno to
    // bind_errnos[0]. We want to avoid this method returning -1 and setting
    // errno to a temporary error (like EAGAIN) repeatedly because proper code
    // should retry on temporary errors, leading to an infinite loop.
    constexpr std::array<int, 4> bind_errnos{{
        EACCES,
        EADDRINUSE,
        EADDRNOTAVAIL,
        EAGAIN,
...
int FuzzedSock::Listen(int) const {
    // Have a permanent error at listen_errnos[0] because when the fuzzed data
    // is exhausted SetFuzzedErrNo() will always set the global errno to
    // listen_errnos[0]. We want to avoid this method returning -1 and setting
    // errno to a temporary error (like EAGAIN) repeatedly because proper code
    // should retry on temporary errors, leading to an infinite loop.
    constexpr std::array<int, 3> listen_errnos{{
        EADDRINUSE,
        EINVAL,
        EOPNOTSUPP,
    }};
...
int FuzzedSock::GetSockName(sockaddr *name, socklen_t *name_len) const {
    constexpr std::array<int, 2> getsockname_errnos{{
        ECONNRESET,
        ENOBUFS,
    }};
    if (m_fuzzed_data_provider.ConsumeBool()) {
