 ...
 * Coins whose output index is 4 (mod 5) have GetCoin() always succeed after
 * being spent. This exercises code paths with spent, non-DIRTY cache entries.
 */
...
public:
    ...
    void BatchWrite(CoinsViewCacheCursor &cursor, const BlockHash &) final {
    ...
    LIMITED_WHILE(provider.remaining_bytes(), 10000) {
        ...
        CallOneOf(
            ...
            [&]() {
                ...
                // Apply to real caches.
                caches.back()->Flush(
                    /*reallocate_cache=*/provider.ConsumeBool());
            },
