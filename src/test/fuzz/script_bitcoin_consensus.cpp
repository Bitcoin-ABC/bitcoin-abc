...
FUZZ_TARGET(script_bitcoin_consensus) {
    ...
    const std::vector<uint8_t> random_bytes_1 =
        ConsumeRandomLengthByteVector(fuzzed_data_provider);
    const std::vector<uint8_t> random_bytes_2 =
        ConsumeRandomLengthByteVector(fuzzed_data_provider);
    const int64_t money = fuzzed_data_provider.ConsumeIntegralInRange<int64_t>(
        0, MAX_MONEY_AS_INT);
    bitcoinconsensus_error err;
    bitcoinconsensus_error *err_p =
        fuzzed_data_provider.ConsumeBool() ? &err : nullptr;
    ...
    (void)bitcoinconsensus_verify_script_with_amount(
        random_bytes_1.data(), random_bytes_1.size(), money,
        random_bytes_2.data(), random_bytes_2.size(), n_in, flags, err_p);
}
