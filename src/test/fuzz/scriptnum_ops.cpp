...
bool IsValidAddition(const CScriptNum &lhs, const CScriptNum &rhs) {
    return rhs == 0 ||
           (rhs > 0 &&
            lhs <= CScriptNum{std::numeric_limits<int64_t>::max()} - rhs) ||
           (rhs < 0 &&
            lhs >= CScriptNum{std::numeric_limits<int64_t>::min()} - rhs);
}
    ...
    while (fuzzed_data_provider.remaining_bytes() > 0) {
        switch (fuzzed_data_provider.ConsumeIntegralInRange(0, 11)) {
            case 0: {
                ...
                // overflow: -2261405121394637306 + -9223372036854775802 cannot
                // be represented in type 'long'
                if (IsValidAddition(script_num, CScriptNum{i})) {
                    assert((script_num + i) - i == script_num);
                }
                // Avoid signed integer overflow:
                // script/script.h:265:93: runtime error: signed integer
                // overflow: 9223371895120855039 - -9223372036854710486 cannot
                // be represented in type 'long'
            ...
            case 1: {
                const CScriptNum random_script_num =
                    ConsumeScriptNum(fuzzed_data_provider);
                assert((script_num == random_script_num) !=
                ...
                // Avoid signed integer overflow:
                // script/script.h:264:93: runtime error: signed integer
                // overflow: -9223126527765971126 + -9223372036854756825 cannot
                // be represented in type 'long'
                ...
                // Avoid signed integer overflow:
                // script/script.h:265:93: runtime error: signed integer
                // overflow: 6052837899185946624 - -9223372036854775808 cannot
                // be represented in type 'long'
            ...
            case 2: {
                ...
                if (!IsValidAddition(script_num, random_script_num)) {
                    ...
                    // ./script/script.h:292: CScriptNum
                    // &CScriptNum::operator+=(const int64_t &): Assertion `rhs
                    // == 0 || (rhs > 0 && m_value <=
                    // std::numeric_limits<int64_t>::max() - rhs) || (rhs < 0 &&
            ...
            case 3: {
                ...
                if (!IsValidSubtraction(script_num, random_script_num)) {
                    // Avoid assertion failure:
                    // ./script/script.h:300: CScriptNum
                    // &CScriptNum::operator-=(const int64_t &): Assertion `rhs
                    // == 0 || (rhs > 0 && m_value >=
                    // std::numeric_limits<int64_t>::min() + rhs) || (rhs < 0 &&
            ...
            case 7:
                if (script_num ==
                    ...
                    // Avoid assertion failure:
                    // ./script/script.h:279: CScriptNum CScriptNum::operator-()
                    // const: Assertion `m_value !=
                    // std::numeric_limits<int64_t>::min()' failed.
                    break;
                }
                script_num = -script_num;
                break;
            ...
            case 10: {
                const int64_t random_integer =
                    fuzzed_data_provider.ConsumeIntegral<int64_t>();
                if (!IsValidSubtraction(script_num,
        ...
        // script/script.h:332:35: runtime error: negation of
        // -9223372036854775808 cannot be represented in type 'int64_t' (aka
