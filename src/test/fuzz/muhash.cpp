...
constexpr std::array<const uint8_t, 768> MODULUS_BYTES = {
    {155, 40,  239, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
...
FUZZ_TARGET(num3072_mul) {
    ...
    // TODO use std::ranges::equal(buf_num, buf_uint) after C++20
    for (size_t i = 0; i < 384; i++) {
        assert(buf_num[i] == buf_uint[i]);
    }
}
...
FUZZ_TARGET(muhash) {
    ...
    const uint256 initial_state_hash = uint256S(
    ...
    CallOneOf(
        ...
        [&] {
            // Test that MuHash result is consistent independent of order of
            // operations
            muhash.Finalize(out);
        ...
        [&] {
            // Test that multiplication with the initial state never changes the
            // finalized result
            muhash.Finalize(out);
            MuHash3072 muhash3;
            muhash3 *= muhash;
...
#pragma clang diagnostic pop
        ...
        [&] {
            // Test that removing all added elements brings the object back to
            // it's initial state
            muhash.Remove(data);
            muhash.Remove(data2);
            muhash.Finalize(out);
