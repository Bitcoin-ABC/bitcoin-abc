...
FUZZ_TARGET(crypto_chacha20) {
    ...
    const auto key = ConsumeFixedLengthByteVector<std::byte>(
        fuzzed_data_provider, ChaCha20::KEYLEN);
    ...
    while (fuzzed_data_provider.ConsumeBool()) {
        CallOneOf(
            ...
            [&] {
                auto key = ConsumeFixedLengthByteVector<std::byte>(
                    fuzzed_data_provider, ChaCha20::KEYLEN);
                chacha20.SetKey(key);
            },
            [&] {
            ...
            [&] {
                std::vector<std::byte> output(
                    fuzzed_data_provider.ConsumeIntegralInRange<size_t>(0,
                                                                        4096));
                const auto input = ConsumeFixedLengthByteVector<std::byte>(
...
template <bool UseCrypt> void ChaCha20SplitFuzz(FuzzedDataProvider &provider) {
    ...
    auto key_bytes =
        ConsumeFixedLengthByteVector<std::byte>(provider, ChaCha20::KEYLEN);
    ...
    // If using Crypt(), initialize data1 and data2 with the same
    // InsecureRandomContext based stream.
    if constexpr (UseCrypt) {
        InsecureRandomContext(provider.ConsumeIntegral<uint64_t>())
            .fillrand(data1);
...
FUZZ_TARGET(crypto_fschacha20) {
    ...
    auto fsc20 = FSChaCha20{
        key, fuzzed_data_provider.ConsumeIntegralInRange<uint32_t>(1, 1024)};

    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 10000) {
        auto input = fuzzed_data_provider.ConsumeBytes<std::byte>(
            fuzzed_data_provider.ConsumeIntegralInRange(0, 4096));
        std::vector<std::byte> output;
