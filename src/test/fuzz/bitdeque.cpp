...
FUZZ_TARGET_INIT(bitdeque, InitRandData) {
    ...
    size_t maxlen =
        (1U << provider.ConsumeIntegralInRange<size_t>(0, LEN_BITS)) - 1;
    ...
    while (provider.remaining_bytes()) {
        ...
        CallOneOf(
            ...
            [&] {
                ...
                auto count = provider.ConsumeIntegralInRange<size_t>(0, maxlen);
                auto rand_begin =
                    RANDDATA.begin() + ctx.randbits(RANDDATA_BITS);
            ...
            [&] {
                ...
                std::initializer_list<bool> ilist{
                    ctx.randbool(), ctx.randbool(), ctx.randbool(),
            ...
            [&] {
                // begin() and end()
                assert(deq.end() - deq.begin() ==
                       bitdeq.end() - bitdeq.begin());
            },
            [&] {
                // begin() and end() (const)
                assert(cdeq.end() - cdeq.begin() ==
                       cbitdeq.end() - cbitdeq.begin());
            },
            [&] {
                // rbegin() and rend()
                assert(deq.rend() - deq.rbegin() ==
                       bitdeq.rend() - bitdeq.rbegin());
            },
            [&] {
                // rbegin() and rend() (const)
                assert(cdeq.rend() - cdeq.rbegin() ==
                       cbitdeq.rend() - cbitdeq.rbegin());
            },
            [&] {
                // cbegin() and cend()
                assert(cdeq.cend() - cdeq.cbegin() ==
                       cbitdeq.cend() - cbitdeq.cbegin());
            },
            [&] {
                // crbegin() and crend()
                assert(cdeq.crend() - cdeq.crbegin() ==
                       cbitdeq.crend() - cbitdeq.crbegin());
            },
            [&] {
                // size() and maxsize()
            ...
            [&] {
                ...
                if (!cdeq.empty()) {
                    size_t pos = provider.ConsumeIntegralInRange<size_t>(
                        0, cdeq.size() - 1);
                    auto &ref = deq.at(pos);
            ...
            [&] {
                // back() const
                if (!cdeq.empty()) {
                    const auto &cdeq2 = deq;
                    const auto &cbitdeq2 = bitdeq;
                    auto &ref = cdeq2.back();
                    auto bitref = cbitdeq2.back();
                    assert(ref == bitref);
                }
            },
            [&] {
            ...
            [&] {
                ...
                if (!cdeq.empty()) {
                    if (cdeq.size() == 1) {
                        ...
                        deq.pop_back();
                        bitdeq.pop_back();
                        // references to other elements are not invalidated
                        assert(ref == bitref);
                    }
                }
            },
            ...
            [&] {
                ...
                if (!cdeq.empty()) {
                    size_t before = provider.ConsumeIntegralInRange<size_t>(
                        0, cdeq.size() - 1);
                    size_t after = cdeq.size() - 1 - before;
                    auto it = deq.erase(cdeq.begin() + before);
                    auto bitit = bitdeq.erase(cbitdeq.begin() + before);
                    assert(it == cdeq.begin() + before &&
                           it == cdeq.end() - after);
                    assert(bitit == cbitdeq.begin() + before &&
                           bitit == cbitdeq.end() - after);
                }
            },
            [&] {
                // erase (at front, range)
                size_t count =
                    provider.ConsumeIntegralInRange<size_t>(0, cdeq.size());
                auto it = deq.erase(cdeq.begin(), cdeq.begin() + count);
                auto bitit =
                    bitdeq.erase(cbitdeq.begin(), cbitdeq.begin() + count);
                assert(it == deq.begin());
            ...
            [&] {
                ...
                if (cdeq.size() < limitlen) {
                    ...
                    auto bitit =
                        do_emplace
                            ? bitdeq.emplace(cbitdeq.begin() + before, val)
            ...
            [&] {
                ...
                if (cdeq.size() < limitlen) {
                    size_t count =
                        provider.ConsumeIntegralInRange<size_t>(0, maxlen);
                    auto rand_begin =
                        RANDDATA.begin() + ctx.randbits(RANDDATA_BITS);
                    auto rand_end = rand_begin + count;
                    auto it = deq.insert(cdeq.begin(), rand_begin, rand_end);
                    auto bitit =
                        bitdeq.insert(cbitdeq.begin(), rand_begin, rand_end);
            ...
            [&] {
                ...
                if (cdeq.size() < limitlen) {
                    ...
                    auto bitit =
                        bitdeq.insert(cbitdeq.end(), rand_begin, rand_end);
            ...
            [&] {
                ...
                if (cdeq.size() < limitlen) {
                    ...
                    auto bitit =
                        bitdeq.insert(cbitdeq.begin() + before, count, val);
                    assert(it == deq.begin() + before);
                    assert(bitit == bitdeq.begin() + before);
                }
            },
            [&] {
                ...
                if (cdeq.size() < limitlen) {
                    ...
                    auto rand_end = rand_begin + count;
                    auto it =
                        deq.insert(cdeq.begin() + before, rand_begin, rand_end);
                    auto bitit = bitdeq.insert(cbitdeq.begin() + before,
                                               rand_begin, rand_end);
                    assert(it == deq.begin() + before);
                    assert(bitit == bitdeq.begin() + before);
                }
