...
BOOST_AUTO_TEST_CASE(damaged_private_key) {
    ...
    const auto i2p_private_key_file =
        m_args.GetDataDirNet() / "test_i2p_private_key_damaged";

    for (const auto &[file_contents, expected_error] :
         std::vector<std::tuple<std::string, std::string>>{
             ...
             {std::string(385, '\0') + '\0' + '\1',
              "Certificate length (1) designates that the private key should "
              "be 388 bytes, but it is only "
              "387 bytes"},

             {std::string(385, '\0') + '\0' + '\5' + "abcd",
              "Certificate length (5) designates that the private key should "
              "be 392 bytes, but it is only "
              "391 bytes"}}) {
