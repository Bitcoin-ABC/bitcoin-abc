...
struct HitRateTest : BasicTestingSetup {
     ...
     * would be 0.5. Therefore, if we see a hit rate of 0.4, 0.4*2.0 = 0.8 is
     * the normalized hit rate.
     *
     * This is basically the right semantics, but has a bit of a glitch
     * depending on how you measure around load 1.0 as after load 1.0 your
     * normalized hit rate becomes effectively perfect, ignoring freshness.
     */
...
struct EraseTest : BasicTestingSetup {
    /**
     * This helper checks that erased elements are preferentially inserted onto
     * and that the hit rate of "fresher" keys is reasonable.
     */
    template <typename Cache> void test_cache_erase(size_t megabytes) {
        ...
        // Check that we have a more than 2x better hit rate on stale elements
        // than erased elements.
...
struct GenerationsTest : BasicTestingSetup {
    template <typename Cache> void test_cache_generations() {
        // This test checks that for a simulation of network activity, the fresh
        // hit rate is never below 99%, and the number of times that it is worse
        // than 99.9% are less than 1% of the time.
        double min_hit_rate = 0.99;
        double tight_hit_rate = 0.999;
        double max_rate_less_than_tight_hit_rate = 0.01;
        // A cache that meets this specification is therefore shown to have a
        // hit rate of at least tight_hit_rate * (1 -
        ...
        // iterations with non-deterministic values, so it isn't "overfit" to
        // the specific entropy in FastRandomContext(true) and implementation of
        // the cache.
        SeedRandomForTest(SeedRand::ZEROS);

        // block_activity models a chunk of network activity. n_insert elements
        // are added to the cache. The first and last n/4 are stored for removal
        // later and the middle n/2 are not stored. This models a network which
        // uses half the signatures of recently (since the last block) added
        ...
        struct block_activity {
            ...
            block_activity(uint32_t n_insert, Cache &c) : reads() {
                ...
                for (uint32_t i = n_insert - (n_insert / 4); i < n_insert;
                     ++i) {
                    reads.push_back(inserts[i]);
                }
        ...
        // we use the deque last_few to model a sliding window of blocks. at
        // each step, each of the last WINDOW_SIZE block_activities checks the
        // cache for POP_AMOUNT of the hashes that they inserted, and marks
        // these erased.
        for (uint32_t i = 0; i < total; ++i) {
            ...
            // behavior on the first WINDOW_SIZE iterations where the deque is
            // not full yet.
            ...
            // Tighter check, count number of times we are less than
            // tight_hit_rate (and implicitly, greater than min_hit_rate)
