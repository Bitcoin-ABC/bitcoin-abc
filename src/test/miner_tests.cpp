...
void TestPackageSelection(const CChainParams &chainparams, CScript scriptPubKey,
    ...
    tx.vin[0].prevout.utxid = txFirst[0]->GetUtxid(MALFIX_MODE_MEMPOOL);
    ...
    utxid_t utxidParentTx = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
    mempool.addUnchecked(
        hashParentTx,
        entry.Fee(1000).Time(GetTime()).SpendsCoinbase(true).FromTx(tx));

    // This tx has a medium fee: 10000 satoshis.
    tx.vin[0].prevout.utxid = txFirst[1]->GetUtxid(MALFIX_MODE_MEMPOOL);
    ...
    tx.vin[0].prevout.utxid = utxidParentTx;
    // 50k satoshi fee.
    tx.vout[0].nValue = 5000000000LL - 1000 - 50000;
    txid_t hashHighFeeTx = tx.GetId();
    utxid_t utxidHighFeeTx = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
    mempool.addUnchecked(
        hashHighFeeTx,
    ...
    tx.vin[0].prevout.utxid = utxidHighFeeTx;
    // 0 fee.
    tx.vout[0].nValue = 5000000000LL - 1000 - 50000;
    txid_t hashFreeTx = tx.GetId();
    utxid_t utxidFreeTx = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
    ...
    tx.vin[0].prevout.utxid = utxidFreeTx;
    tx.vout[0].nValue = 5000000000LL - 1000 - 50000 - feeToUse;
    txid_t hashLowFeeTx = tx.GetId();
    ...
    tx.vin[0].prevout.utxid = txFirst[2]->GetUtxid(MALFIX_MODE_MEMPOOL);
    tx.vout.resize(2);
    tx.vout[0].nValue = 5000000000LL - 100000000;
    // 1BCC output.
    tx.vout[1].nValue = 100000000;
    txid_t hashFreeTx2 = tx.GetId();
    utxid_t utxidFreeTx2 = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
    mempool.addUnchecked(hashFreeTx2,
                         entry.Fee(0).SpendsCoinbase(true).FromTx(tx));

    // This tx can't be mined by itself.
    tx.vin[0].prevout.utxid = utxidFreeTx2;
    tx.vout.resize(1);
    feeToUse = blockMinFeeRate.GetFee(freeTxSize);
    tx.vout[0].nValue = 5000000000LL - 100000000 - feeToUse;
    txid_t hashLowFeeTx2 = tx.GetId();
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity) {
    ...
    for (unsigned int i = 0; i < 1001; ++i) {
        ...
        mempool.addUnchecked(hash, entry.Fee(LOWFEE)
                                       ...
                                       .FromTx(tx));
        tx.vin[0].prevout.utxid = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
    }
    BOOST_CHECK_THROW(
        BlockAssembler(config, chainparams).CreateNewBlock(scriptPubKey),
        std::runtime_error);
    ...
    for (unsigned int i = 0; i < 1001; ++i) {
        tx.vout[0].nValue -= LOWFEE;
        hash = tx.GetId();
        utxid = tx.GetUtxid(MALFIX_MODE_MEMPOOL);
        // Only first tx spends coinbase.
        bool spendsCoinbase = (i == 0) ? true : false;
    ...
    tx.vin[1].prevout.utxid = txFirst[0]->GetUtxid(MALFIX_MODE_MEMPOOL);
    ...
    tx.vin[0].prevout.utxid = txFirst[3]->GetUtxid(MALFIX_MODE_MEMPOOL);
