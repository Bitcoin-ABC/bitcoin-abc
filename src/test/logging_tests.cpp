...
static constexpr std::streamsize FLUSH_SENTINEL_LOG_SIZE{33};
...
BOOST_AUTO_TEST_CASE(logging_log_rate_limiter) {
    ...
    scheduler.m_service_thread =
        std::thread([&scheduler] { scheduler.serviceQueue(); });
    uint64_t max_bytes{1024};
    auto reset_window{1min};
    auto sched_func = [&scheduler](auto func, auto window) {
...
BOOST_AUTO_TEST_CASE(logging_log_limit_stats) {
    ...
    BOOST_CHECK_EQUAL(counter.GetAvailableBytes(),
...
void LogFromLocationAndExpect(int location, std::string message,
                              std::string expect) {
...
BOOST_FIXTURE_TEST_CASE(logging_filesize_rate_limit, LogSetup) {
    ...
    auto limiter =
        std::make_unique<BCLog::LogRateLimiter>(sched_func, 1024 * 1024, 20s);
    ...
    std::string utf8_path{LogInstance().m_file_path.u8string()};
    const char *log_path{utf8_path.c_str()};

    // Use GetFileSize because fs::file_size may require a flush to be accurate.
    std::streamsize log_file_size{
        static_cast<std::streamsize>(GetFileSize(log_path))};

    // Logging 1 MiB should be allowed.
    for (int i = 0; i < 1024; ++i) {
        LogFromLocation(0, log_message);
    }
    BOOST_CHECK_MESSAGE(log_file_size < GetFileSize(log_path),
                        "should be able to log 1 MiB from location 0");
    ...
    BOOST_CHECK_MESSAGE(log_file_size < GetFileSize(log_path),
                        "the start of the suppression period should be logged");
    ...
    BOOST_CHECK_MESSAGE(log_file_size == GetFileSize(log_path),
                        "all further logs from location 0 should be dropped");

    BOOST_CHECK_THROW(
        LogFromLocationAndExpect(1, log_message, "Excessive logging detected"),
        std::runtime_error);

    // FIXME: the flush prints to the log file so we account for these extra
    // characters.
    flush_debug_log();
    BOOST_CHECK_MESSAGE(log_file_size + FLUSH_SENTINEL_LOG_SIZE <
                            GetFileSize(log_path),
                        "location 1 should be unaffected by other locations");
    ...
    BOOST_CHECK_MESSAGE(log_file_size + FLUSH_SENTINEL_LOG_SIZE <
                            GetFileSize(log_path),
                        "the end of the suppression period should be logged");

    BOOST_CHECK_THROW(
        LogFromLocationAndExpect(1, log_message, "Restarting logging"),
        std::runtime_error);

    // Attempt to log 1MiB from location 2 and 1MiB from location 3. These
    // exempt locations should be allowed to log without limit.
    flush_debug_log();
    log_file_size = GetFileSize(log_path);
    for (int i = 0; i < 1024; ++i) {
        BOOST_CHECK_THROW(LogFromLocationAndExpect(
                              2, log_message, "Excessive logging detected"),
                          std::runtime_error);
    }
    ...
    BOOST_CHECK_MESSAGE(log_file_size + FLUSH_SENTINEL_LOG_SIZE <
                            GetFileSize(log_path),
                        "location 2 should be exempt from rate limiting");

    log_file_size = GetFileSize(log_path);
    for (int i = 0; i < 1024; ++i) {
        BOOST_CHECK_THROW(LogFromLocationAndExpect(
                              3, log_message, "Excessive logging detected"),
                          std::runtime_error);
    }
    ...
    BOOST_CHECK_MESSAGE(log_file_size + FLUSH_SENTINEL_LOG_SIZE <
                            GetFileSize(log_path),
                        "location 3 should be exempt from rate limiting");
