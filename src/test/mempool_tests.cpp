...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest)
{
    ...
    pool.addUnchecked(tx1.GetId(), entry.Fee(10000LL).Priority(10.0).FromTx(tx1));
    ...
    pool.addUnchecked(tx2.GetId(), entry.Fee(20000LL).Priority(9.0).FromTx(tx2));
    ...
    pool.addUnchecked(tx4.GetId(), entry.Fee(15000LL).Priority(1.0).FromTx(tx4));
    ...
    pool.addUnchecked(tx8.GetId(), entry.Fee(0LL).Time(2).FromTx(tx8), setAncestors);
    ...
    pool.addUnchecked(tx9.GetId(), entry.Fee(0LL).Time(3).FromTx(tx9), setAncestors);
    ...
    sortedOrder.insert(sortedOrder.begin()+7, tx10.GetId().ToString()); // tx10 is just before tx6
    CheckSort<descendant_score>(pool, sortedOrder);
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest)
{
    ...
    pool.TrimToSize(GetTransactionSize(tx1)); // mempool is limited to tx1's size in memory usage, so nothing fits
    BOOST_CHECK(!pool.exists(tx1.GetId()));
    BOOST_CHECK(!pool.exists(tx2.GetId()));
    BOOST_CHECK(!pool.exists(tx3.GetId()));

    CFeeRate maxFeeRateRemoved(25000, GetTransactionSize(tx3) + GetTransactionSize(tx2));
