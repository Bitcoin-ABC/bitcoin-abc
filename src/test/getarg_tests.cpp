...
void ResetArgs(ArgsManager &local_args, const std::string &strArg) {
    ...
    std::string error;
    BOOST_CHECK(
        local_args.ParseParameters(vecChar.size(), vecChar.data(), error));
}
...
BOOST_AUTO_TEST_CASE(boolarg) {
    ...
    // -nofoo should win
    ResetArgs(local_args, "-foo -nofoo");
    BOOST_CHECK(!local_args.GetBoolArg("-foo", false));
    BOOST_CHECK(!local_args.GetBoolArg("-foo", true));

    // -nofoo should win
    ResetArgs(local_args, "-foo=1 -nofoo=1");
    BOOST_CHECK(!local_args.GetBoolArg("-foo", false));
    BOOST_CHECK(!local_args.GetBoolArg("-foo", true));

    // -nofoo=0 should win
    ResetArgs(local_args, "-foo=0 -nofoo=0");
    BOOST_CHECK(local_args.GetBoolArg("-foo", false));
    BOOST_CHECK(local_args.GetBoolArg("-foo", true));
...
BOOST_AUTO_TEST_CASE(patharg) {
    ...
    ResetArgs(local_args, "");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir", "default"),
                      fs::path{"default"});
...
BOOST_AUTO_TEST_CASE(boolargno) {
    ...
    // foo always wins:
    ResetArgs(local_args, "-nofoo -foo");
    BOOST_CHECK(local_args.GetBoolArg("-foo", true));
    BOOST_CHECK(local_args.GetBoolArg("-foo", false));
}
