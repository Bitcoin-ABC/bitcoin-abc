...
public:
    explicit IndexReorgCrash(std::unique_ptr<interfaces::Chain> chain,
                             std::shared_future<void> blocker,
                             int blocking_height)
        : BaseIndex(std::move(chain), "test index"), m_blocker(blocker),
          m_blocking_height(blocking_height) {
        const fs::path path = gArgs.GetDataDirNet() / "index";
        fs::create_directories(path);
        m_db = std::make_unique<BaseIndex::DB>(path / "db", /*n_cache_size=*/0,
                                               /*f_memory=*/true,
                                               /*f_wipe=*/false);
    }
    ...
    // FIXME Replace with CustomAppend when backporting PR25494 (see PR32878)
    bool WriteBlock(const CBlock &block, const CBlockIndex *pindex) override {
        const auto block_height = pindex->nHeight;

        // Simulate a delay so new blocks can get connected during the initial
        // sync
        if (block_height == m_blocking_height) {
            m_blocker.wait();
        }

        // Move mock time forward so the best index gets updated only when we
        // are not at the blocking height
        if (block_height == m_blocking_height - 1 ||
            block_height > m_blocking_height) {
...
BOOST_FIXTURE_TEST_CASE(index_reorg_crash, BuildChainTestingSetup) {
    ...
    int blocking_height = WITH_LOCK(
        cs_main, return m_node.chainman->ActiveChain().Tip()->nHeight);

    IndexReorgCrash index(
        interfaces::MakeChain(m_node, m_node.chainman->GetParams()), blocker,
        blocking_height);
    ...
    auto func_wait_until = [&](int height, std::chrono::milliseconds timeout) {
        ...
        while (index.GetSummary().best_block_height < height) {
            if (std::chrono::steady_clock::now() > deadline) {
                BOOST_FAIL(strprintf(
                    "Timeout waiting for index height %d (current: %d)", height,
    ...
    const CBlockIndex *prev_tip =
        WITH_LOCK(cs_main, return m_node.chainman->ActiveChain().Tip()->pprev);
    CKey key;
    key.MakeNewKey(/*fCompressed=*/true);
    BOOST_REQUIRE(BuildChain(
        prev_tip, GetScriptForDestination(PKHash(key.GetPubKey())), 3, fork));
