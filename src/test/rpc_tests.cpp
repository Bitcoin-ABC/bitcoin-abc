...
BOOST_AUTO_TEST_CASE(json_parse_errors) {
    ...
    BOOST_CHECK_EQUAL(ParseNonRFCJSONValue("{\"a\": true}")["a"].get_bool(),
                      true);
    BOOST_CHECK_EQUAL(ParseNonRFCJSONValue("{\"1\": \"true\"}")["1"].get_str(),
                      "true");
    ...
    BOOST_CHECK_THROW(ParseNonRFCJSONValue("{1: \"true\"}"),
                      std::runtime_error);
    ...
    BOOST_CHECK_THROW(ParseNonRFCJSONValue("{{\"a\": \"a\"}: 1}"),
                      std::runtime_error);
