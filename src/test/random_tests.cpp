...
BOOST_AUTO_TEST_CASE(fastrandom_tests_nondeterministic) {
    ...
    {
        BOOST_CHECK(FastRandomContext().rand<uint64_t>() !=
                    uint64_t{9330418229102544152u});
        BOOST_CHECK(FastRandomContext().rand<int>() != int{618925161});
        BOOST_CHECK(FastRandomContext()
                        .randrange<std::chrono::microseconds>(1h)
                        .count() != 1271170921);
        BOOST_CHECK(FastRandomContext()
                        .randrange<std::chrono::milliseconds>(1h)
...
BOOST_AUTO_TEST_CASE(randbits_test) {
    ...
    for (int i = 0; i < 5; ++i) {
        // count (first) how often it has occurred, and (second) how often it
        // was true:
        // - for every bit position, in every requested bits count
        //   (0 + 1 + 2 +... + 64 = 2080)
        ...
        while (true) {
            ...
            for (int j = 0; j < 1000; ++j) {
                // Decide on a number of bits to request (0 through 64,
                ...
                for (int bit = 0; bit < bits; ++bit) {
                    int idx = bit + (bits * (bits - 1)) / 2 +
                              2080 * ctx_test_bitsleft;
                    seen[idx].first += 1;
            ...
            if (std::all_of(seen.begin(), seen.end(),
                            [](const auto &x) { return x.first >= 242; })) {
                break;
            }
        }
        ...
        for (const auto &val : seen) {
            assert(fabs(val.first * 0.5 - val.second) <
                   sqrt(val.first * 0.25) * 7.78);
        }
    }
}
