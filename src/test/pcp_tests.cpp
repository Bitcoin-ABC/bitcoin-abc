...
struct TestOp {
    ...
    //! Injected error.
    //! Set this field to a non-zero value to return the errno code from send or
    //! receive operation.
    int error;

    TestOp(std::chrono::milliseconds delay_in, Op op_in,
           const std::vector<uint8_t> &data_in, int error_in)
...
public:
    explicit PCPTestingSetup(const ChainType chainType = ChainType::MAIN,
        ...
        const std::optional<CService> local_ipv4{
            Lookup("192.168.0.6", 1, false)};
        const std::optional<CService> local_ipv6{
            Lookup("2a10:1234:5678:9abc:def0:1234:5678:9abc", 1, false)};
        const std::optional<CService> gateway_ipv4{
            Lookup("192.168.0.1", 1, false)};
        const std::optional<CService> gateway_ipv6{
            Lookup("2a10:1234:5678:9abc:def0:0000:0000:0000", 1, false)};
    ...
    static constexpr PCPMappingNonce TEST_NONCE{{0x11, 0x22, 0x33, 0x44, 0x55,
...
public:
    ...
    explicit PCPTestSock(const CNetAddr &local_ip, const CNetAddr &gateway_ip,
                         const std::vector<TestOp> &script)
        : Sock{INVALID_SOCKET}, m_script(script), m_local_ip(local_ip),
    ...
    ssize_t Send(const void *data, size_t len, int) const override {
        ...
        if (AtEndOfScript() || CurOp().op != TestOp::SEND) {
            // Ignore sends after end of script, or sends when we expect a
            // receive.
            FailScript();
            return len;
        }
    ...
    ssize_t Recv(void *buf, size_t len, int flags) const override {
        if (!m_connected || AtEndOfScript() || CurOp().op != TestOp::RECV ||
            m_time_left != 0s) {
            return -1;
        }
        if (CurOp().error) {
    ...
    int Connect(const sockaddr *sa, socklen_t sa_len) const override {
        ...
        if (service.SetSockAddr(sa, sa_len) &&
            service == CService(m_gateway_ip, 5351)) {
            // If bind-any, bind to local ip.
            if (m_bound.IsBindAny()) {
                m_bound = CService(m_local_ip, 0);
            }
            // If no port assigned, assign port 1.
            if (m_bound.GetPort() == 0) {
    ...
    bool Wait(std::chrono::milliseconds timeout, Event requested,
        ...
        if (AtEndOfScript() || requested != Sock::RECV) {
            ...
            ElapseTime(delay);
            m_time_left -= delay;
            if (CurOp().op == TestOp::RECV && m_time_left == 0s &&
                occurred != nullptr) {
                *occurred = Sock::RECV;
            }
...
private:
    ...
    mutable std::chrono::milliseconds m_time{
        MockableSteadyClock::INITIAL_MOCK_TIME};
...
BOOST_AUTO_TEST_CASE(natpmp_ipv4) {
    ...
    CreateSock = [this, &script](int domain, int type, int protocol) {
        if (domain == AF_INET && type == SOCK_DGRAM &&
            ...
            return std::make_unique<PCPTestSock>(default_local_ipv4,
                                                 default_gateway_ipv4, script);
        }
...
BOOST_AUTO_TEST_CASE(pcp_ipv4) {
    const std::vector<TestOp> script{
        {0ms,
         TestOp::SEND,
         {
             ...
             0x00, 0xff, 0xff, 0xc0, 0xa8, 0x00, 0x06, // internal IP
             0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99,
             0xaa, 0xbb, 0xcc,       // nonce
             0x06, 0x00, 0x00, 0x00, // protocol (TCP), reserved
             0x04, 0xd2, 0x04, 0xd2, // internal port, suggested external port
             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
             0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, // suggested external IP
         },
         0},
        {250ms,
         TestOp::RECV, // 250ms delay before answer
         {
             ...
             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
             0x00, 0xff, 0xff, 0x01, 0x02, 0x03, 0x04, // assigned external IP
         },
         0},
    };
    CreateSock = [this, &script](int domain, int type, int protocol) {
    ...
    auto res = PCPRequestPortMap(TEST_NONCE, default_gateway_ipv4,
                                 bind_any_ipv4, 1234, 1000, 1, 1000ms);
...
BOOST_AUTO_TEST_CASE(pcp_ipv6) {
    const std::vector<TestOp> script{
        {0ms,
         TestOp::SEND,
         {
             0x02, 0x01, 0x00, 0x00, // version, opcode
             0x00, 0x00, 0x03, 0xe8, // lifetime
             0x2a, 0x10, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde,
             0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, // internal IP
             0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99,
             0xaa, 0xbb, 0xcc,       // nonce
             0x06, 0x00, 0x00, 0x00, // protocol (TCP), reserved
             0x04, 0xd2, 0x04, 0xd2, // internal port, suggested external port
             0x2a, 0x10, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde,
             0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, // suggested external IP
         },
         0},
        {500ms,
         TestOp::RECV, // 500ms delay before answer
         {
             0x02, 0x81, 0x00, 0x00, // version, opcode, result success
             0x00, 0x00, 0x01, 0xf4, // granted lifetime
             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    CreateSock = [this, &script](int domain, int type, int protocol) {
        if (domain == AF_INET6 && type == SOCK_DGRAM &&
            protocol == IPPROTO_UDP) {
            return std::make_unique<PCPTestSock>(default_local_ipv6,
                                                 default_gateway_ipv6, script);
        }
        return std::unique_ptr<PCPTestSock>();
    };

    auto res = PCPRequestPortMap(TEST_NONCE, default_gateway_ipv6,
                                 default_local_ipv6, 1234, 1000, 1, 1000ms);

    MappingResult *mapping = std::get_if<MappingResult>(&res);
    BOOST_REQUIRE(mapping);
    BOOST_CHECK_EQUAL(mapping->version, 2);
    BOOST_CHECK_EQUAL(mapping->internal.ToStringAddrPort(),
                      "[2a10:1234:5678:9abc:def0:1234:5678:9abc]:1234");
    BOOST_CHECK_EQUAL(mapping->external.ToStringAddrPort(),
                      "[2a10:1234:5678:9abc:def0:1234:5678:9abc]:1234");
    BOOST_CHECK_EQUAL(mapping->lifetime, 500);
}
...
BOOST_AUTO_TEST_CASE(pcp_timeout) {
    ...
    auto res = PCPRequestPortMap(TEST_NONCE, default_gateway_ipv4,
                                 bind_any_ipv4, 1234, 1000, 3, 2000ms);
...
BOOST_AUTO_TEST_CASE(pcp_ipv6_timeout_success) {
    const std::vector<TestOp> script{
        {0ms,
         ...
         0},
        {2001ms,
         TestOp::NOP, // Takes longer to respond than timeout of 2000ms
         {},
         0},
        {0ms,
         TestOp::SEND, // Repeated send (try 2)
         {
             0x02, 0x01, 0x00, 0x00, // version, opcode
             0x00, 0x00, 0x03, 0xe8, // lifetime
             0x2a, 0x10, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde,
         ...
         0},
        {200ms,
         TestOp::RECV, // This time we're in time
         {
             0x02, 0x81, 0x00, 0x00, // version, opcode, result success
             0x00, 0x00, 0x01, 0xf4, // granted lifetime
             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    auto res = PCPRequestPortMap(TEST_NONCE, default_gateway_ipv6,
                                 default_local_ipv6, 1234, 1000, 2, 2000ms);
...
BOOST_AUTO_TEST_CASE(pcp_ipv4_fail_no_resources) {
    const std::vector<TestOp> script{
        ...
        {500ms,
         TestOp::RECV,
         {
             0x02, 0x81, 0x00, 0x08, // version, opcode, result 0x08: no
                                     // resources
             ...
             0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
             0x00, 0x00, 0x00, 0x00, 0x00, // assigned external IP
         },
         0},
    };
    CreateSock = [this, &script](int domain, int type, int protocol) {
    ...
    auto res = PCPRequestPortMap(TEST_NONCE, default_gateway_ipv4,
                                 bind_any_ipv4, 1234, 1000, 3, 1000ms);
