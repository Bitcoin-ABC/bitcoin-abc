...
struct CacheTest : BasicTestingSetup {
    // This is a large randomized insert/remove simulation test on a
    // variable-size stack of caches on top of CCoinsViewTest.
    //
    // It will randomly create/update/delete Coin entries to a tip of caches,
    // with txids picked from a limited list of random 256-bit hashes.
    // Occasionally, a new tip is added to the stack of caches, or the tip is
    // flushed and removed.
    //
    // During the process, booleans are kept to make sure that the randomized
    // operation hits all branches.
    //
    // If fake_best_block is true, assign a random BlockHash to mock the
    // recording of best block on flush. This is necessary when using
    // CCoinsViewDB as the base, otherwise we'll hit an assertion in BatchWrite.
    //
    void SimulationTest(CCoinsView *base, bool fake_best_block) {
        ...
        // Use a limited set of random transaction ids, so we do test
        // overwriting entries.
        std::vector<TxId> txids;
        txids.resize(NUM_SIMULATION_ITERATIONS / 8);
        for (size_t i = 0; i < txids.size(); i++) {
            txids[i] = TxId(m_rng.rand256());
        }
        ...
        for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
            // Do a random modification.
            {
                ...
                const TxId txid = txids[m_rng.randrange(txids.size())];
                Coin &coin = result[COutPoint(txid, 0)];
                // Determine whether to test HaveCoin before or after Access*
                // (or both). As these functions can influence each other's
                // behaviour by pulling things into the cache, all combinations
                // are tested.
                ...
                // Infrequently, test usage of AccessByTxid instead of
                // AccessCoin - the former just delegates to the latter and
                // returns the first unspent in a txn.
                const Coin &entry =
                ...
                if (m_rng.randrange(5) == 0 || coin.IsSpent()) {
                    ...
                    if (m_rng.randrange(16) == 0 && coin.IsSpent()) {
                        txout.scriptPubKey.assign(1 + m_rng.randbits(6),
                                                  OP_RETURN);
                        BOOST_CHECK(txout.scriptPubKey.IsUnspendable());
                        added_an_unspendable_entry = true;
                    } else {
                        // Random sizes so we can test memory usage accounting
                        txout.scriptPubKey.assign(m_rng.randbits(6), 0);
                        (coin.IsSpent() ? added_an_entry : updated_an_entry) =
                            true;
                        coin = Coin(txout, 1, false);
                    }

                    Coin newcoin(txout, 1, false);
                    if (COutPoint op(txid, 0);
                        !stack.back()->map().contains(op) &&
                        !newcoin.GetTxOut().scriptPubKey.IsUnspendable() &&
                        m_rng.randbool()) {
                        stack.back()->EmplaceCoinInternalDANGER(
                            std::move(op), std::move(newcoin));
                    } else {
                        stack.back()->AddCoin(
                            op, std::move(newcoin),
                            /*possible_overwrite=*/!coin.IsSpent() ||
                                m_rng.randbool());
                    }
                } else {
                    // Spend the coin.
                ...
                if (stack.size() > 1 && m_rng.randbool() == 0) {
                    ...
                    if (fake_best_block) {
                        stack[flushIndex]->SetBestBlock(
                            BlockHash(m_rng.rand256()));
                    }
                    bool should_erase = m_rng.randrange(4) < 3;
                    should_erase ? stack[flushIndex]->Flush()
            ...
            if (m_rng.randrange(100) == 0) {
                ...
                if (stack.size() > 0 && m_rng.randbool() == 0) {
                    ...
                    if (fake_best_block) {
                        stack.back()->SetBestBlock(BlockHash(m_rng.rand256()));
                    }
...
struct UpdateTest : BasicTestingSetup {
    ...
    UtxoData::iterator FindRandomFrom(const std::set<COutPoint> &utxoSet) {
        assert(utxoSet.size());
        auto utxoSetIt =
            utxoSet.lower_bound(COutPoint(TxId(m_rng.rand256()), 0));
...
BOOST_FIXTURE_TEST_CASE(updatecoins_simulation_test, UpdateTest) {
    ...
    for (int64_t i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            // Random sizes so we can test memory usage accounting
            tx.vout[0].scriptPubKey.assign(m_rng.rand32() & 0x3F, 0);
        ...
        if (utxoset.size() > 1 && m_rng.randrange(30) == 0) {
            stack[m_rng.rand32() % stack.size()]->Uncache(
...
static void WriteCoinsViewEntry(CCoinsView &view, const MaybeCoin &cache_coin) {
    ...
    if (cache_coin) {
        InsertCoinMapEntry(map, sentinel, *cache_coin);
    }
    size_t dirty_count{cache_coin && cache_coin->IsDirty()};
    auto cursor{
        CoinsViewCacheCursor(dirty_count, sentinel, map, /*will_erase=*/true)};
    view.BatchWrite(cursor, BlockHash{});
...
struct FlushTest : BasicTestingSetup {
    Coin MakeCoin() {
        ...
        Coin coin{CTxOut{RandMoney(m_rng), std::move(scriptPubKey)},
                  /*nHeightIn=*/static_cast<uint32_t>(m_rng.randrange(4096)),
                  /*IsCoinbase=*/false};
        return coin;
    }
    ...
    //! 5. (if `do_erasing_flush`) Ensure the entry has been written to the
    //! parent
    ...
    void TestFlushBehavior(
        ...
        auto flush_all = [this, &all_caches](bool erase) {
            // Flush in reverse order to ensure that flushes happen from
            // children up.
            for (auto i = all_caches.rbegin(); i != all_caches.rend(); ++i) {
                auto &cache = *i;
                cache->SanityCheck();
                // hashBlock must be filled before flushing to disk; value is
                // unimportant here. This is normally done during
                // connect/disconnect block.
                cache->SetBestBlock(BlockHash{m_rng.rand256()});
        ...
        // `base` shouldn't have coin (no flush yet) but `view` should have
        // cached it.
        BOOST_CHECK(!base.HaveCoin(outp));
        BOOST_CHECK(view->HaveCoin(outp));
        ...
        // --- 3. Ensuring the entry still exists in the cache and has been
        // written to parent
        //
        ...
        BOOST_CHECK(!view->HaveCoin(
            outp)); // Coin should be considered spent in `view`.
        BOOST_CHECK(
        ...
        // --- Bonus check: ensure that a coin added to the base view via one
        // cache
        ...
        // --- Bonus check 2: ensure that a FRESH, spent coin is deleted by
        // Sync()
        //
        txid = TxId{m_rng.rand256()};
        outp = COutPoint(txid, 0);
...
BOOST_AUTO_TEST_CASE(ccoins_reset_guard) {
    ...
    BlockHash base_best_block{m_rng.rand256()};
