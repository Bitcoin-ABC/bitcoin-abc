...
BOOST_FIXTURE_TEST_CASE(chainstatemanager, TestChain100Setup) {
    ...
    mineBlocks(10);
    BOOST_CHECK_EQUAL(
        WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 110);
    ...
    mineBlocks(1);
    BOOST_CHECK_EQUAL(
        WITH_LOCK(manager.GetMutex(), return manager.ActiveHeight()), 111);
...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    std::tuple<Chainstate *, Chainstate *> SetupSnapshot() {
        ...
        // Mine 10 more blocks, putting at us height 110 where a valid
        // assumeutxo value can be found.
        ...
        const auto &au_data =
            chainman.GetParams().AssumeutxoForHeight(snapshot_height);
        const CBlockIndex *tip =
            WITH_LOCK(chainman.GetMutex(), return chainman.ActiveTip());

        BOOST_CHECK_EQUAL(tip->m_chain_tx_count, au_data->m_chain_tx_count);

        // To be checked against later when we try loading a subsequent
        // snapshot.
        BlockHash loaded_snapshot_blockhash{*chainman.SnapshotBlockhash()};

        // Make some assertions about the both chainstates. These checks ensure
        // the legacy chainstate hasn't changed and that the newly created
        // chainstate reflects the expected content.
        {
            LOCK(::cs_main);
            int chains_tested{0};

            for (Chainstate *chainstate : chainman.GetAll()) {
                BOOST_TEST_MESSAGE("Checking coins in "
                                   << chainstate->ToString());
            ...
            for (Chainstate *chainstate : chainman.GetAll()) {
                ...
                for (CTransactionRef &txn : m_coinbase_txns) {
                    ...
                    if (coinscache.HaveCoin(op)) {
                        (is_background ? coins_in_background
                                       : coins_in_active)++;
...
//!
//! - Then mark a region of the chain as missing data and introduce a second
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_loadblockindex, TestChain100Setup) {
    ...
    for (int i = 0; i <= cs1.m_chain.Height(); ++i) {
        ...
        if (i < last_assumed_valid_idx && i >= assumed_valid_start_idx) {
            index->nStatus = BlockStatus().withValidity(BlockValidity::TREE);
            index->nTx = 0;
    ...
    // 121 total blocks, including genesis
    BOOST_CHECK_EQUAL(num_indexes, 121);
    // original chain has height 120
    BOOST_CHECK_EQUAL(assumed_tip->nHeight, 120);
    // current cs1 chain has height 90
    BOOST_CHECK_EQUAL(validated_tip->nHeight, 90);
    // current cs2 chain has height 110
    BOOST_CHECK_EQUAL(assumed_base->nHeight, 110);

    // Regenerate cs1.setBlockIndexCandidates and cs2.setBlockIndexCandidate
    // and check contents below.
    reload_all_block_indexes();
    ...
    //   LoadBlockIndex has a special case to always add the snapshot block as
    //   a candidate. The special case is only actually intended to apply to
    //   the snapshot chainstate cs2, not the background chainstate cs1, but it
    //   is written broadly and applies to both.
    //
    // - It does not have any blocks after height 110 because cs1 is a
    //   background chainstate, and only blocks where are ancestors of the
    //   snapshot block are added as candidates for the background chainstate.
    ...
    BOOST_CHECK_EQUAL(cs2.setBlockIndexCandidates.count(assumed_base->pprev),
                      0);
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_init, SnapshotTestSetup) {
    ...
    BlockValidationState unused_state;
    {
        ...
        // to avoid queuedTx assertion errors on teardown
        unused_pool.clear();
    }
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_completion,
    ...
    // chainstate.
    BOOST_CHECK(active_cs.m_coinstip_cache_size_bytes >
                tip_cache_before_complete);
    BOOST_CHECK(active_cs.m_coinsdb_cache_size_bytes >
                db_cache_before_complete);
...

    {
        ...
        BOOST_CHECK(!chainman_restarted.IsSnapshotActive());
        BOOST_CHECK(!chainman_restarted.IsSnapshotValidated());
        BOOST_CHECK(active_cs2.m_coinstip_cache_size_bytes >
                    tip_cache_before_complete);
        BOOST_CHECK(active_cs2.m_coinsdb_cache_size_bytes >
                    db_cache_before_complete);
...
/**
 * Helper function to parse args into args_man and return the result of
 * applying them to opts
 */
...
util::Result<Options> SetOptsFromArgs(ArgsManager &args_man, Options opts,
    ...
    if (!args_man.ParseParameters(argv.size(), argv.data(), error)) {
        return util::Error{
            Untranslated("ParseParameters failed with error: " + error)};
    }
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_args, BasicTestingSetup) {
    ...
    auto get_opts = [&](const std::vector<const char *> &args) {
        ...
        static const ChainstateManager::Options options{
            .config = config, .datadir = {}, .notifications = notifications};
        return SetOptsFromArgs(*this->m_node.args, options, args);
    };
    //! Like get_opts, but requires the provided args to be valid and unwraps
    //! the result
    ...
    BOOST_CHECK(
        get_valid_opts({"-assumevalid="}).assumed_valid_block.value().IsNull());
    BOOST_CHECK(get_valid_opts({"-assumevalid=0"})
                    .assumed_valid_block.value()
                    .IsNull());
    BOOST_CHECK(get_valid_opts({"-noassumevalid"})
                    .assumed_valid_block.value()
                    .IsNull());
    BOOST_CHECK_EQUAL(get_valid_opts({"-assumevalid=0x1234"})
                          .assumed_valid_block.value()
                          .ToString(),
                      std::string(60, '0') + "1234");
    ...
    BOOST_CHECK_EQUAL(get_valid_opts({"-minimumchainwork=0"})
                          .minimum_chain_work.value()
                          .GetCompact(),
                      0U);
    BOOST_CHECK_EQUAL(get_valid_opts({"-nominimumchainwork"})
                          .minimum_chain_work.value()
                          .GetCompact(),
                      0U);
    BOOST_CHECK_EQUAL(get_valid_opts({"-minimumchainwork=0x1234"})
                          .minimum_chain_work.value()
                          .GetCompact(),
                      0x02123400U);
