...
static void CheckGetPruneHeight(node::BlockManager &blockman, CChain &chain,
                                int height)
    ...
    for (CBlockIndex *it{chain[height]}; it != nullptr && it->nHeight > 0;
         it = it->pprev) {
...
BOOST_FIXTURE_TEST_CASE(get_prune_height, TestChain100Setup) {
    ...
    // Fresh chain of 100 blocks without any pruned blocks, so std::nullopt
    // should be returned
...
BOOST_FIXTURE_TEST_CASE(invalidate_block, TestChain100Setup) {
    ...
    WITH_LOCK(::cs_main, assert(tip_to_invalidate->nStatus.isInvalid()));
    WITH_LOCK(::cs_main, assert(!tip_to_invalidate->nStatus.hasFailedParent()));

    // check all ancestors of the invalidated block are validated up to
    // BLOCK_VALID_TRANSACTIONS and are not invalid
    auto pindex = tip_to_invalidate->pprev;
    while (pindex) {
        WITH_LOCK(::cs_main,
                  assert(pindex->IsValid(BlockValidity::TRANSACTIONS)));
        WITH_LOCK(::cs_main, assert(!pindex->nStatus.isInvalid()));
        pindex = pindex->pprev;
    }
    ...
    while (pindex && pindex != tip_to_invalidate) {
        WITH_LOCK(::cs_main, assert(!pindex->nStatus.hasFailed()));
        WITH_LOCK(::cs_main, assert(pindex->nStatus.hasFailedParent()));
        pindex = pindex->pprev;
    }
}
...
BOOST_AUTO_TEST_CASE(num_chain_tx_max) {
    ...
    BOOST_CHECK_EQUAL(block_index.m_chain_tx_count,
