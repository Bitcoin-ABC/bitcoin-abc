...
BOOST_AUTO_TEST_CASE(basic_allocating) {
    ...
    // case the call is forwarded to operator new 0 bytes takes one entry from
    // the first freelist
    ...
    for (size_t i = 0; i < 1000; ++i) {
        ...
        if (ptr_size_alignment.empty() || 0 != m_rng.randrange(4)) {
            ...
            std::size_t alignment = std::size_t{1}
                                    << m_rng.randrange(8); // 1, 2, ..., 128
            std::size_t size = (m_rng.randrange(200) / alignment + 1) *
                               alignment; // multiple of alignment
            void *ptr = resource.Allocate(size, alignment);
            BOOST_TEST(ptr != nullptr);
            BOOST_TEST((reinterpret_cast<uintptr_t>(ptr) & (alignment - 1)) ==
                       0);
            ptr_size_alignment.push_back({ptr, size, alignment});
        } else {
            // deallocate a random item
            auto &x =
                ptr_size_alignment[m_rng.randrange(ptr_size_alignment.size())];
