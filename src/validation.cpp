...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package &package,
    ...
    // mempool, and we don't want to reject the entire package in that case (as
    // that could be a censorship vector).  Filter the transactions that are
    // already in mempool and add their information to results, since we already
    // have them.
