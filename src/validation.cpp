...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package &package,
    ...
    // Results from individual validation. "Nonfinal" because if a transaction
    // fails by itself but succeeds later (i.e. when evaluated with a
    // fee-bumping child), the result changes (though not reflected in this
    // map). If a transaction fails more than once, we want to return the first
    // result, when it was considered on its own. So changes will only be from
    // invalid -> valid.
    ...
    // Quit early because package validation won't change the result or the
    // entire package has already been submitted.
    if (quit_early || txns_package_eval.empty()) {
        for (const auto &[txid, mempoolaccept_res] :
             individual_results_nonfinal) {
            Assume(results_final.emplace(txid, mempoolaccept_res).second);
            Assume(mempoolaccept_res.m_result_type ==
                   MempoolAcceptResult::ResultType::INVALID);
        }
        return PackageMempoolAcceptResult(package_state_quit_early,
                                          std::move(results_final));
    }
    // Validate the (deduplicated) transactions as a package.  Note that
    // submission_result has its own PackageValidationState;
    // package_state_quit_early is unused past this point.
    ...
    for (const auto &[txid, mempoolaccept_res] : results_final) {
        Assume(submission_result.m_tx_results.emplace(txid, mempoolaccept_res)
                   .second);
        Assume(mempoolaccept_res.m_result_type !=
               MempoolAcceptResult::ResultType::INVALID);
    }
    if (submission_result.m_state.GetResult() ==
        PackageValidationResult::PCKG_TX) {
        // Package validation failed because one or more transactions failed.
        // Provide a result for each transaction;
        // if AcceptMultipleTransactions() didn't return a result for a tx,
        // include the previous individual failure reason.
        submission_result.m_tx_results.insert(
            individual_results_nonfinal.cbegin(),
