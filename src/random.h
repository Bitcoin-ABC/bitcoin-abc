 ...
 *
 * During tests, the RNG can be put into a special deterministic mode, in which
 * the output of all RNG functions, with the exception of GetStrongRandBytes(),
 * is replaced with the output of a deterministic RNG. This deterministic RNG
 * does not gather entropy, and is unaffected by RandAddPeriodic() or
 * RandAddEvent(). It produces pseudorandom data that only depends on the seed
 * it was initialized with, possibly until it is reinitialized.
 */
...
concept RandomNumberGenerator = requires(T &rng, Span<std::byte> s) {
    ...
    // A random number generator must derive from RandomMixin, which adds other
    // rand* functions.
 ...
 * https://en.cppreference.com/w/cpp/language/crtp. An RNG class FunkyRNG would
 * derive publicly from RandomMixin<FunkyRNG>. This permits RandomMixin from
 * accessing the derived class's rand64() function, while also allowing the
 * derived class to provide more.
 *
...
private:
     ...
     *
     * This also enforces the RandomNumberGenerator concept. We cannot declare
     * that in the template (no template<RandomNumberGenerator T>) because the
     * type isn't fully instantiated yet there.
     */
...
public:
    ...
    template <std::integral I> I randrange(I range) noexcept {
    ...
    template <std::integral I> I rand() noexcept {
        static_assert(std::numeric_limits<I>::max() <=
                      std::numeric_limits<uint64_t>::max());
        static constexpr auto BITS =
            std::bit_width(uint64_t(std::numeric_limits<I>::max()));
        static_assert(std::numeric_limits<I>::max() ==
...
public:
    /**
     * Construct a FastRandomContext with GetRandHash()-based entropy (or zero
     * key if fDeterministic).
     */
    ...
    /**
     * Fill a byte Span with random bytes. This overrides the RandomMixin
     * version.
     */
...
/**
 * xoroshiro128++ PRNG. Extremely fast, not appropriate for cryptographic
 * purposes.
 *
...
class InsecureRandomContext : public RandomMixin<InsecureRandomContext> {
    ...
    [[nodiscard]] constexpr static uint64_t
...
public:
    ...
    // no copy - that is dangerous, we don't want accidentally copy the RNG and
    // then have two streams with exactly the same results.
 ...
 */
template <typename T> T GetRand(T nMax) noexcept {
    return T(FastRandomContext().randrange(nMax));
}

/**
 * Generate a uniform random integer of type T in its entire non-negative
 * range.
 */
template <typename T> T GetRand() noexcept {
