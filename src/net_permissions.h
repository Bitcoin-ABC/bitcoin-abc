...
enum class NetPermissionFlags : uint32_t {
    ...
    Implicit = (1U << 31),
    All = BloomFilter | ForceRelay | Relay | NoBan | Mempool | Download |
          BypassProofRequestLimits | Addr,
};
...
public:
    ...
    //! NetPermissionFlags::Relayor NetPermissionFlags::Download, as that
...
public:
    static bool TryParse(const std::string &str,
                         NetWhitebindPermissions &output, bilingual_str &error);
    CService m_service;
};
