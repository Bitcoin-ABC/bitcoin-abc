...
namespace sam {
    ...
    Binary Session::MyDestination() const {
        ...
        if (m_private_key.size() < CERT_LEN_POS + sizeof(cert_len)) {
            throw std::runtime_error(strprintf(
                "The private key is too short (%d < %d)", m_private_key.size(),
        ...
        if (dest_len > m_private_key.size()) {
            throw std::runtime_error(
                strprintf("Certificate length (%d) designates that the private "
                          "key should be %d bytes, but it is only %d bytes",
