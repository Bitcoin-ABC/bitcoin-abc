...
namespace sam {
    ...
    public:
        ...
        /**
         * Construct a transient session which will generate its own I2P private
         * key rather than read the one from disk (it will not be saved on disk
         * either and will be lost once this object is destroyed). This will not
         * initiate any IO, the session will be lazily created later when first
         * used.
         * @param[in] control_host Location of the SAM proxy.
         * @param[in,out] interrupt If this is signaled then all operations are
         * canceled as soon as possible and executing methods throw an
