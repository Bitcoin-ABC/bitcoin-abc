...
void OutputGroup::Insert(const CInputCoin &output, int depth, bool from_me,
    ...
    // m_ancestors is currently the max ancestor count for all coins in the
    // group; however, this is not ideal, as a wallet will consider e.g. thirty
    // 2-ancestor coins as having two ancestors, when in reality it has 60
    // ancestors.
    m_ancestors = std::max(m_ancestors, ancestors);
    // m_descendants is the count as seen from the top ancestor, not the
    // descendants as seen from the coin itself; thus, this value is accurate
