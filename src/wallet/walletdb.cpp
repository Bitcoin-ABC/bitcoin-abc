...
bool CWalletDB::WriteAccount(const string &strAccount,
...
void CWalletDB::ListAccountCreditDebit(const string &strAccount,
                                       list<CAccountingEntry> &entries) {
    ...
    while (true) {
        ...
        if (setRange)
            ssKey << std::make_pair(
                std::string("acentry"),
                std::make_pair((fAllAccounts ? string("") : strAccount),
                               uint64_t(0)));
...
DBErrors CWalletDB::ZapSelectTx(CWallet *pwallet, vector<uint256> &vTxHashIn,
