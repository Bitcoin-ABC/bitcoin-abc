...
class RNGState {
    ...
    /**
     * If not nullopt, the output of this RNGState is redirected and drawn from
     * here (unless always_use_real_rng is passed to MixExtract).
     */
...
public:
    ...
    /**
     * Make the output of MixExtract (unless always_use_real_rng)
     * deterministic, with specified seed.
     */
     ...
     * returned
     *
     * If always_use_real_rng is false, and MakeDeterministic has been called
     * before, output from the deterministic PRNG instead.
     */
...
GetExponentialRand(std::chrono::microseconds now,
    ...
    double unscaled = -std::log1p(FastRandomContext().randbits<48>() *
