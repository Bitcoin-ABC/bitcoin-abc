...
void BenchFillClearMap(benchmark::Bench &bench, Map &map) {
    ...
    // make sure each iteration of the benchmark contains exactly 5000 inserts
    // and one clear.
...
PoolAllocator_StdUnorderedMapWithPoolResource(benchmark::Bench &bench) {
    ...
    // make sure the resource supports large enough pools to hold the node.
    // We do this by adding the size of a few pointers to it.
    auto pool_resource = Map::allocator_type::ResourceType();
    auto map = Map{0, std::hash<uint64_t>{}, std::equal_to<uint64_t>{},
                   &pool_resource};
    BenchFillClearMap(bench, map);
}
