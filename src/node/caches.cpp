...
CacheSizes CalculateCacheSizes(const ArgsManager &args, size_t n_indexes) {
    // Convert -dbcache from MiB units to bytes. The total cache is floored by
    // MIN_DB_CACHE and capped by max size_t value.
    ...
    if (std::optional<int64_t> db_cache = args.GetIntArg("-dbcache")) {
        ...
        constexpr auto max_db_cache{sizeof(void *) == 4
                                        ? MAX_32BIT_DBCACHE
    ...
    index_sizes.tx_index = std::min(
        total_cache / 8,
        args.GetBoolArg("-txindex", DEFAULT_TXINDEX) ? MAX_TX_INDEX_CACHE : 0);
