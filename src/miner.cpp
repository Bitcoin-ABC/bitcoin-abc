...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn) {
    ...
    coinbaseTx.vout[1].scriptPubKey = GetScriptForDevReward(*config);
    coinbaseTx.vout[1].nValue = GetBlockRewardSubsidy(nHeight, chainparams.GetConsensus());
...
static bool ProcessBlockFound(const CBlock* pblock, const Config &config)
{
    LogPrintf("%s\n", pblock->ToString());
    LogPrintf("generated %s\n", pblock->vtx[0]->vout.at(0).nValue.ToString());
...
void static BitcoinMiner(const Config &config)
{
//    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    if (chainparams.NetworkIDString().compare("regtest") == 0) {
        LogPrintf("regtest donot support miner\n");
    }
...
//                    }
            ...
            LogPrintf("start to create new block");
            //
            // Create new block
            //
            unsigned int nTransactionsUpdatedLast = g_mempool.GetTransactionsUpdated();
            CBlockIndex* pindexPrev = chainActive.Tip();
...
//                if (vNodes.empty() && chainparams.MiningRequiresPeers())
                if (nNonce >= 0xffff0000)
                    break;
                if (g_mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 60)
                    break;
                if (pindexPrev != chainActive.Tip())
                    break;

                // Update nTime every few seconds
                if (UpdateTime(pblock, config, pindexPrev) < 0)
...
void GenerateBitcoins(bool fGenerate, int nThreads, const Config &config)
{
    ...
    if (minerThreads != NULL) {
        LogPrintf(" stop all miner threads...\n");
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinMiner, boost::cref(config)));
}
