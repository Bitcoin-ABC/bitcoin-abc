...
static inline ConnectionDirection &operator|=(ConnectionDirection &a,
 ...
 *
 * @param      name     The string provided by the user representing a local
 *                      path
 *
 * @returns Whether the string has proper format, length, and points to an
 *          existing file path
 */
...
public:
    ...
    explicit Proxy(const std::string path, bool randomize_credentials = false)
        : m_unix_socket_path(path), m_is_unix_socket(true),
          m_randomize_credentials(randomize_credentials) {}
    ...
    bool IsValid() const {
        if (m_is_unix_socket) {
            return IsUnixSocketPath(m_unix_socket_path);
        }
 ...
 *          string resolved or std::nullopt if host does not resolve to an
 * address.
 *
...
LookupHost(const std::string &name, bool fAllowLookup,
 ...
 * @param[in] manual_connection Whether or not the connection was manually
 * requested (e.g. through the addnode RPC)
 *
 * @returns the connected socket if the operation succeeded, empty unique_ptr
 * otherwise
 */
 ...
 * @param[out] proxy_connection_failed Whether or not the connection to the
 * SOCKS5 proxy failed.
 *
 * @returns the connected socket if the operation succeeded. Otherwise an empty
 * unique_ptr.
 */
