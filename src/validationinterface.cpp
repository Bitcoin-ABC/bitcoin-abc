...
void CMainSignals::RegisterWithMempoolSignals(CTxMemPool &pool) {
    g_connNotifyEntryRemoved.emplace(
        &pool, pool.NotifyEntryRemoved.connect(boost::bind(
...
void RegisterValidationInterface(CValidationInterface *pwalletIn) {
    ...
    conns.TransactionRemovedFromMempool =
        g_signals.m_internals->TransactionRemovedFromMempool.connect(
            boost::bind(&CValidationInterface::TransactionRemovedFromMempool,
                        pwalletIn, _1));
    ...
    conns.Broadcast = g_signals.m_internals->Broadcast.connect(boost::bind(
