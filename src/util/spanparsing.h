 ...
 * braces. So for example, for "foo(bar(1),2),3" the initial part
 * "foo(bar(1),2)" will be returned. sp will be updated to skip the initial part
 * that is returned.
 */
Span<const char> Expr(Span<const char> &sp);

/** Split alias for backwards compatibility */
template <typename... Args> auto Split(Args &&...args) {
