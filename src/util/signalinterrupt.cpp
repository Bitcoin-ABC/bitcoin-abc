...
SignalInterrupt::SignalInterrupt() : m_flag{false} {
    ...
    if (!pipe) {
        throw std::ios_base::failure("Could not create TokenPipe");
    }
...
void SignalInterrupt::reset() {
    // Cancel existing interrupt by waiting for it, this will reset condition
    // flags and remove the token from the pipe.
    if (*this) {
        wait();
    }
    m_flag = false;
...
void SignalInterrupt::operator()() {
    ...
    // a condition variable is not safe. Make sure that the token is only
    // written once even if multiple threads call this concurrently or in case
    // of a reentrant signal.
