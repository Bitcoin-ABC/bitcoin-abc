...
// template<class M> using Result = std::expected<M, Error>;
// after C++23 is enforced.
...
public:
    ...
    Result() : m_variant{std::in_place_index_t<1>{}, std::monostate{}} {}
    Result(T obj) : m_variant{std::in_place_index_t<1>{}, std::move(obj)} {}
    Result(Error error)
        : m_variant{std::in_place_index_t<0>{}, std::move(error.message)} {}
