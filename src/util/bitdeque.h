 ...
 */
template <int BlobSize = 4096 * 8> class bitdeque {
    ...
    template <bool Const> class Iterator {
        using deque_iterator =
            std::conditional_t<Const, typename deque_type::const_iterator,
                               typename deque_type::iterator>;

        deque_iterator m_it;
        int m_bitpos{0};
        Iterator(const deque_iterator &it, int bitpos)
            : m_it(it), m_bitpos(bitpos) {}
    ...
    public:
        ...
        using reference =
            std::conditional_t<Const, bool, typename word_type::reference>;
        ...
        friend bool operator<(const Iterator &x, const Iterator &y) {
            return std::tie(x.m_it, x.m_bitpos) < std::tie(y.m_it, y.m_bitpos);
        }
        friend bool operator>(const Iterator &x, const Iterator &y) {
            return std::tie(x.m_it, x.m_bitpos) > std::tie(y.m_it, y.m_bitpos);
        }
        ...
        friend bool operator==(const Iterator &x, const Iterator &y) {
            return x.m_it == y.m_it && x.m_bitpos == y.m_bitpos;
        }
        friend bool operator!=(const Iterator &x, const Iterator &y) {
            return x.m_it != y.m_it || x.m_bitpos != y.m_bitpos;
        }
...
private:
    ...
    void erase_back(size_type n) {
        if (n >= static_cast<size_type>(BITS_PER_WORD - m_pad_end)) {
            ...
            m_pad_end = 0;
            m_deque.erase(m_deque.end() - 1 - (n / BITS_PER_WORD),
                          m_deque.end());
            n %= BITS_PER_WORD;
        }
        if (n) {
    ...
    void erase_front(size_type n) {
        if (n >= static_cast<size_type>(BITS_PER_WORD - m_pad_begin)) {
            ...
            m_deque.erase(m_deque.begin(),
                          m_deque.begin() + 1 + (n / BITS_PER_WORD));
            n %= BITS_PER_WORD;
        }
        if (n) {
    ...
    void insert_zeroes(size_type before, size_type count) {
        ...
        if (before < after) {
            ...
            extend_back(count);
            std::move_backward(begin() + before, begin() + before + after,
                               end());
        }
    }
...
public:
    ...
    bool empty() const noexcept {
        return m_deque.size() == 0 ||
               (m_deque.size() == 1 &&
                (m_pad_begin + m_pad_end == BITS_PER_WORD));
    }

    /** Return the maximum size of the container. */
    size_type max_size() const noexcept {
        if (m_deque.max_size() <
            std::numeric_limits<difference_type>::max() / BITS_PER_WORD) {
    ...
    template <typename It> bitdeque(It first, It last) { assign(first, last); }
    ...
    reference at(size_type position) {
        if (position >= size())
            throw std::out_of_range("bitdeque::at() out of range");
    ...
    void shrink_to_fit() { m_deque.shrink_to_fit(); }
    ...
    iterator erase(iterator pos) {
        return erase(const_iterator{pos}, const_iterator{pos} + 1);
    }
