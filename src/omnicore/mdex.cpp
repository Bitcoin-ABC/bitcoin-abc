    ...
    for (md_PricesMap::iterator priceIt = ppriceMap->begin(); priceIt != ppriceMap->end(); ++priceIt) { // check all prices
        ...
        while (offerIt != pofferSet->end()) { // specific price, check all properties
            ...
            if (IsFeatureActivated(FEATURE_FEES, pnew->getBlock())) {
                if (pold->getProperty() > OMNI_PROPERTY_TWHC && pold->getDesProperty() > OMNI_PROPERTY_TWHC) {
...
std::string CMPMetaDEx::displayUnitPrice() const
{
     ...
     if (getDesProperty() == OMNI_PROPERTY_WHC || getDesProperty() == OMNI_PROPERTY_TWHC) {
         ...
         if (isPropertyDivisible(getProperty())) tmpDisplayPrice = tmpDisplayPrice * COIN.GetSatoshis();
     } else {
         tmpDisplayPrice = inversePrice();
         if (isPropertyDivisible(getDesProperty())) tmpDisplayPrice = tmpDisplayPrice * COIN.GetSatoshis();
     }
...
std::string CMPMetaDEx::displayFullUnitPrice() const
{
    ...
    /* Matching types require no action (divisible/divisible or indivisible/indivisible)
       Non-matching types require adjustment for display purposes
           divisible/indivisible   : *COIN.GetSatoshis()
           indivisible/divisible   : /COIN.GetSatoshis()
    */
    if ( isPropertyDivisible(getProperty()) && !isPropertyDivisible(getDesProperty()) ) tempUnitPrice = tempUnitPrice*COIN.GetSatoshis();
    if ( !isPropertyDivisible(getProperty()) && isPropertyDivisible(getDesProperty()) ) tempUnitPrice = tempUnitPrice/COIN.GetSatoshis();
