...
std::vector<CService> Lookup(const std::string &name, uint16_t portDefault,
    ...
    const std::vector<CNetAddr> addresses{LookupIntern(
        hostname, nMaxSolutions, fAllowLookup, dns_lookup_function)};
    if (addresses.empty()) {
        return {};
    }
...
std::optional<CService> Lookup(const std::string &name, uint16_t portDefault,
    ...
    return services.empty() ? std::nullopt
                            : std::make_optional(services.front());
}
...
CService LookupNumeric(const std::string &name, uint16_t portDefault,
    ...
    return Lookup(name, portDefault, /*fAllowLookup=*/false,
                  dns_lookup_function)
        .value_or(CService{});
}
...
bool IsUnixSocketPath(const std::string &name) {
    ...
    return str.size() + 1 <= sizeof(((sockaddr_un *)nullptr)->sun_path);
#else
    return false;
#endif
}
...
std::unique_ptr<Sock> CreateSockOS(int domain, int type, int protocol) {
    ...
    if (protocol == IPPROTO_TCP) {
        // Set the no-delay option (disable Nagle's algorithm) on the TCP
        // socket.
        const int on{1};
...
static bool ConnectToSocket(const Sock &sock, struct sockaddr *sockaddr,
    ...
    if (sock.Connect(sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEINPROGRESS || nErr == WSAEWOULDBLOCK ||
            ...
            Sock::Event occurred;
            if (!sock.Wait(std::chrono::milliseconds{nConnectTimeout},
                           requested, &occurred)) {
            ...
            if (sockerr != 0) {
                LogConnectFailure(manual_connection,
                                  ...
                                  dest_str, NetworkErrorString(sockerr));
                return false;
            }
        }
#ifdef WIN32
...
std::unique_ptr<Sock> Proxy::Connect() const {
    ...
    const std::string path{
        m_unix_socket_path.substr(ADDR_PREFIX_UNIX.length())};
