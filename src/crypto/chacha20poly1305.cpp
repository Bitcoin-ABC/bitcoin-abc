...
void AEADChaCha20Poly1305::Encrypt(Span<const std::byte> plain,
    ...
    m_chacha20.Crypt(UCharCast(plain.data()), UCharCast(cipher.data()),
                     plain.size());

    // Seek to block 0, and compute tag using key drawn from there.
    m_chacha20.Seek64(nonce, 0);
    ComputeTag(m_chacha20, aad, cipher.first(plain.size()),
...
bool AEADChaCha20Poly1305::Decrypt(Span<const std::byte> cipher,
    ...
    if (timingsafe_bcmp_internal(UCharCast(expected_tag),
                                 UCharCast(cipher.data() + plain.size()),
                                 EXPANSION)) {
        return false;
    }

    // Decrypt (starting at block 1).
    m_chacha20.Crypt(UCharCast(cipher.data()), UCharCast(plain.data()),
                     plain.size());
    return true;
}
...
bool FSChaCha20Poly1305::Decrypt(Span<const std::byte> cipher,
                                 ...
                                 Span<std::byte> plain) noexcept {
    bool ret =
        m_aead.Decrypt(cipher, aad, {m_packet_counter, m_rekey_counter}, plain);
    NextPacket();
    return ret;
}
