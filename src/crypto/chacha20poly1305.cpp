...
/**
 * Compute poly1305 tag. chacha20 must be set to the right nonce, block 0. Will
 * be at block 1 after.
 */
...
void AEADChaCha20Poly1305::Encrypt(Span<const std::byte> plain1,
    ...
    m_chacha20.Crypt(UCharCast(plain1.data()), UCharCast(cipher.data()),
                     plain1.size());
    m_chacha20.Crypt(UCharCast(plain2.data()),
                     UCharCast(cipher.data() + plain1.size()), plain2.size());
...
bool AEADChaCha20Poly1305::Decrypt(Span<const std::byte> cipher,
    ...
    m_chacha20.Crypt(UCharCast(cipher.data()), UCharCast(plain1.data()),
                     plain1.size());
    m_chacha20.Crypt(UCharCast(cipher.data() + plain1.size()),
                     UCharCast(plain2.data()), plain2.size());
    return true;
}
