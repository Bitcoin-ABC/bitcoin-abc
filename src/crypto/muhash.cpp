...
struct Num3072Signed {
    /**
     * The represented value is sum(limbs[i]*2^(SIGNED_LIMB_SIZE*i),
     * i=0..SIGNED_LIMBS-1).
    ...
    /**
     * Convert a Num3072Signed to a Num3072.
 ...
 * TODO Replace with std::countr_zero after C++20.
 */
 ...
 *
 * The matrix t must be chosen such that t*(f,g) results in multiples of
 * 2^SIGNED_LIMB_SIZE. This is the case for matrices computed by
 * ComputeDivstepMatrix().
 */
...
Num3072 MuHash3072::ToNum3072(Span<const uint8_t> in) {
    ...
    ChaCha20Aligned(hashed_in.data(), hashed_in.size())
        .Keystream64(tmp, Num3072::BYTE_SIZE / 64);
    Num3072 out{tmp};
