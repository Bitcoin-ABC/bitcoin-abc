...
public:
    ...
    /**
     * Block size (inputs/outputs to Keystream / Crypt should be multiples of
     * this).
     */
     ...
     *
     * Block_counter selects a position to seek to (to byte
     * BLOCKLEN*block_counter). After 256 GiB, the block counter overflows, and
     * nonce.first is incremented.
     */
    void Seek(Nonce96 nonce, uint32_t block_counter) noexcept;

    /**
     * outputs the keystream into out, whose length must be a multiple of
     * BLOCKLEN.
     */
     ...
     *
     * The size of input and output must be equal, and be a multiple of
     * BLOCKLEN.
     */
...
public:
    ...
    /**
     * en/deciphers the message <in_bytes> and write the result into
     * <out_bytes>
     *
     * The size of in_bytes and out_bytes must be equal.
     */
    void Crypt(Span<const std::byte> in_bytes,
 ...
 *
 * This implements a stream cipher that automatically transitions to a new
 * stream with a new key and new nonce after a predefined number of encryptions
 * or decryptions.
 *
...
public:
    ...
    /**
     * Construct an FSChaCha20 cipher that rekeys every rekey_interval Crypt()
     * calls.
     */
