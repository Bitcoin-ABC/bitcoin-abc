...
ChaCha20Poly1305AEAD::ChaCha20Poly1305AEAD(const uint8_t *K_1, size_t K_1_len,
...
bool ChaCha20Poly1305AEAD::Crypt(
    ...
    // crypt the AAD (3 bytes message length) with given position in AAD cipher
    // instance keystream
...
bool ChaCha20Poly1305AEAD::GetLength(uint32_t *len24_out, uint64_t seqnr_aad,
                                     int aad_pos, const uint8_t *ciphertext) {
    // enforce valid aad position to avoid accessing outside of the 64byte
    // keystream cache (there is space for 21 times 3 bytes)
    assert(aad_pos >= 0 &&
           aad_pos < CHACHA20_ROUND_OUTPUT - CHACHA20_POLY1305_AEAD_AAD_LEN);
    if (m_cached_aad_seqnr != seqnr_aad) {
        // we need to calculate the 64 keystream bytes since we reached a new
        // aad sequence number
        m_cached_aad_seqnr = seqnr_aad;
    ...
    // decrypt the ciphertext length by XORing the right position of the 64byte
    // keystream cache with the ciphertext
