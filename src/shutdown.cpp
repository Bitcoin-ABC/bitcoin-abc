...
void StartShutdown() {
    ...
    // so using a condition variable is not safe.
    // Make sure that the token is only written once even if multiple threads
    // call this concurrently or in case of a reentrant signal.
    if (!fRequestShutdown.exchange(true)) {
        ...
        while (true) {
            ...
            if (result < 0) {
                // Failure. It's possible that the write was interrupted by
                // another signal.
...
void AbortShutdown() {
    if (fRequestShutdown) {
        // Cancel existing shutdown by waiting for it, this will reset condition
        // flags and remove the shutdown token from the pipe.
        WaitForShutdown();
    }
