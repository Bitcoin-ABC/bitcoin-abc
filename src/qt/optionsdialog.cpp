...
/* Network elements init */
    ...
    for (const QString &langStr : translations.entryList()) {
        ...
        if (langStr.contains("_")) {
            /**
             * display language strings as "native language - native
             * country/territory (locale name)", e.g.
...
void OptionsDialog::updateDefaultProxyNets() {
    ...
    if (!IsUnixSocketPath(proxyIpText)) {
        const std::optional<CNetAddr> ui_proxy_netaddr{
            LookupHost(proxyIpText, /*fAllowLookup=*/false)};
    ...
    ui->proxyReachIPv4->setChecked(has_proxy &&
                                   proxy.ToString() == proxyIpText);

    has_proxy = model->node().getProxy(NET_IPV6, proxy);
    ui->proxyReachIPv6->setChecked(has_proxy &&
                                   proxy.ToString() == proxyIpText);
