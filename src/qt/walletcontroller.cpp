...
void WalletController::removeWallet(WalletModel *wallet_model) {
    // Once the wallet is successfully removed from the node, the model will
    // emit the 'WalletModel::unload' signal. This signal is already connected
    // and will complete the removal of the view from the GUI. Look at
    // 'WalletController::getOrCreateWallet' for the signal connection.
    ...
    if (!m_error_message.empty()) {
        ...
        QMessageBox::warning(
            ...
            QString::fromStdString(
                Join(m_warning_message, Untranslated("\n")).translated));
    }
