 ...
 */
export class Wallet extends WalletBase<KeypairData> {
     ...
     */
    protected _deriveKeypair(forChange: boolean, index: number): KeypairData {
        if (!this.isHD || !this.baseHdNode) {
...
class WalletAction {
    ...
    private _build(config: BuildConfig): BuiltAction {
        ...
        let inputSats = WalletBase.sumUtxosSats(selectedUtxos);
