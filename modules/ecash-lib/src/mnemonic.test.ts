...
const englishVectors: [string, string, string][] = [
    [
...
describe('mnemonic', async () => {
    ...
    for (const [entropy, phrase, seed] of englishVectors) {
        it('mnemonic ' + entropy, () => {
            ...
            expect(toHex(mnemonicToEntropy(phrase, wordlist.words))).to.equal(
                entropy,
            );
            expect(toHex(mnemonicToSeed(phrase, 'TREZOR'))).to.equal(seed);
        });
    }

    it('mnemonic failure', () => {
        expect(() => entropyToMnemonic(new Uint8Array(10), wordlist)).to.throw(
            'Entropy must be between 16 and 32 bytes long',
        );
        expect(() => entropyToMnemonic(new Uint8Array(40), wordlist)).to.throw(
            'Entropy must be between 16 and 32 bytes long',
        );
        expect(() => entropyToMnemonic(new Uint8Array(21), wordlist)).to.throw(
            'Entropy length must be divisible by 4',
        );

        expect(() => mnemonicToEntropy('', wordlist.words)).to.throw(
            'Number of words in mnemonic phrase must be between 12 and 24',
        );
        expect(() =>
            mnemonicToEntropy(
                new Array(25).fill('hans').join(' '),
                wordlist.words,
            ),
        ).to.throw(
            'Number of words in mnemonic phrase must be between 12 and 24',
        ...
        expect(() =>
            mnemonicToEntropy(
                new Array(13).fill('hans').join(' '),
                wordlist.words,
            ),
        ).to.throw('Number of words in mnemonic phrase must be divisible by 3');
        expect(() =>
            mnemonicToEntropy(
                new Array(12).fill('thank').join(' '),
                wordlist.words,
            ),
        ).to.throw('Invalid checksum: expected 6, got e');
    });
});
