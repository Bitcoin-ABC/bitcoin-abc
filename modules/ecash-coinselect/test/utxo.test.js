...
it('calcByteCount() returns a correct byte count for a p2pkh tx with 1 input and 2 outputs', function () {
    const inputCount = 1;
    const outputCount = 2;
    const rawTxHex =
    ...
    const byteCount = calcByteCount(inputCount, outputCount);

    // Verify byte count
    // As per https://bitcoin.stackexchange.com/questions/1195/how-to-calculate-transaction-size-before-sending-legacy-non-segwit-p2pkh-p2sh
    // the correct byte count can be within plus or minus the number of inputs
    // and may not be an exact match for rawTxHex.length / 2
    const rawTxHexBytes = rawTxHex.length / 2;
    const upperByteBound = rawTxHexBytes + inputCount;
    const lowerByteBound = rawTxHexBytes - inputCount;
    assert.equal(byteCount <= upperByteBound, true);
    assert.equal(byteCount >= lowerByteBound, true);
});

it('calcByteCount() returns a correct byte count for a p2pkh tx with 20 inputs and 1 output', function () {
    const inputCount = 20;
    const outputCount = 1;
    const rawTxHex =
    ...
    // Verify byte count
    // As per https://bitcoin.stackexchange.com/questions/1195/how-to-calculate-transaction-size-before-sending-legacy-non-segwit-p2pkh-p2sh
    // the actual byte count can be within plus or minus the number of inputs
    // and may not be an exact match for rawTxHex.length / 2
    const rawTxHexBytes = rawTxHex.length / 2;
    const upperByteBound = rawTxHexBytes + inputCount;
    const lowerByteBound = rawTxHexBytes - inputCount;
...
it('calcByteCount() returns a correct byte count for a p2pkh tx with 6 inputs and 4 outputs', function () {
    const inputCount = 6;
    const outputCount = 4;
    const rawTxHex =
