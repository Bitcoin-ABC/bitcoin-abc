...
over /Applications/Bitcoin-Qt (on Mac) or bitcoinabc/bitcoin-qt (on Linux).
...
bitcoinabc makes it easier for experienced open source developers to contribute 
to the project.
...
Another change in the 0.9 release is moving away from the bitcoinabc executable
functioning both as a server and as a RPC client. The RPC client functionality
("tell the running bitcoin daemon to do THIS") was split into a separate
executable, 'bitcoin-cli'. The RPC client code will eventually be removed from
bitcoinabc, but will be kept for backwards compatibility for a release or two.
...
- Add '-disablewallet' mode to let bitcoinabc run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with bitcoinabc
