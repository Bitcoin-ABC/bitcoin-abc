...
packages need to build host utilities that are used in the
build process.
...
causes two of the devault executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the devault code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the devault source code).
