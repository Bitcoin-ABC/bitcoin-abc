...
On-chain protocol for [proofofwriting.com](https://proofofwriting.com/) feed actions, article publishes, paywall unlocks, and wallet-auth logins.
...
`version` and `action` are bare opcodes (`OP_0`, `OP_1`..`OP_8`).
...
- `nonce` is a 36-byte pushdata (push opcode `0x24`): the ASCII bytes of a standard UUID string (server-issued; single-use).
- `contentHash` is sha256 of the stored UTF-8 content.
- `unlock` carries no payload — minimal marker `6a04504f575200 57`.
